...
fil_tab <- readRDS("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/Syn/Exome_filt3_nCH_C5eqC4_nonmds_iskrisc_05Sept_rect_ASP_syn.rds")
 ...
 # print(max(ftemp_tab$comb_score))
  if(dim(ftemp_tab) == 0 ){
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      sam_gene_gt <- ftemp_tab[ftemp_tab$VARIANT %in% ftemp_tab_var_id[m],][,c(1:3,9,11)]
      ...
      samp_vec[[m]] <- c(ftemp_tab_var_id[m],
                         ...
                         unique(sam_gene_gt$vep_consequence), as.numeric(maf_vec_case),
                         as.numeric(maf_vec_cont), as.numeric(maf_vec), sam10)
    }
    
    samp_vec_mat <- do.call("rbind.data.frame", samp_vec)
    print(dim(samp_vec_mat))
    colnames(samp_vec_mat) <- c("VARIANT", "gene_symbol", 
                                "vep_consequence", "coh_MAF_case",
                                "coh_MAF_cont", "coh_MAF", Ex_samp_id)
    ...
    ##compute maf SKAT takes care of the AFs internally by assigning higher weights to rare variants ; weights.beta
    
    else {
     # gene_mat_comb <- as.numeric(as.character(samp_vec_mat[,5]))
      gene_mat <- as.matrix(samp_vec_mat[,-c(1:6)])
      class(gene_mat) <- "numeric"
      ##for strictly burden test set rho = 1, else set rho = 0
      ##no covariate
      skat_pvals[[i]] <- SKAT_run(geno_mat = gene_mat, gene = genes[i], x=NULL, p_vec, cust_weight = NULL, rho = 1)
      ##only pc1 + pc2
      skat_pvals_pc12[[i]] <- SKAT_run(geno_mat = gene_mat, gene = genes[i], x=p_Data_noCH[,c(19:21)], p_vec, cust_weight = NULL, rho = 1)
      ##gender
      skat_pvals_sex[[i]] <- SKAT_run(geno_mat = gene_mat, gene = genes[i], x=p_Data_noCH$gender, p_vec, cust_weight = NULL, rho = 1)
      ##gender + pc1 + pc2
      skat_pvals_sex_pc12[[i]] <- SKAT_run(geno_mat = gene_mat, gene = genes[i], x=p_Data_noCH[,c(39,19:21)], p_vec, cust_weight = NULL, rho = 1)
...
saveRDS(DT_skat_snv_str_pc123, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/Syn/Exome_skat_filt3_nCH_C5eqC4_nonmds_iskrisc_05Sept_rect_ASP_syn.rds", compress = T)
