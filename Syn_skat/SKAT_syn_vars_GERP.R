...
fil_tab <- fil_tab[!is.na(fil_tab$GERP_SCORE),]
fil_tab <- fil_tab[fil_tab$GERP_SCORE > 1,]
dim(fil_tab)

##remove all variants without GERP score (340 variants lost)
...
for(i in 1:length(genes)){
  ...
  if(dim(ftemp_tab) == 0 ){
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      ##include GERP score
      sam_gene_gt <- ftemp_tab[ftemp_tab$VARIANT %in% ftemp_tab_var_id[m],][,c(1:3,9,11,58)]
      ...
      samp_vec[[m]] <- c(ftemp_tab_var_id[m],
                         ...
                         as.numeric(maf_vec_cont), as.numeric(maf_vec), 
                         as.numeric(unique(sam_gene_gt$GERP_SCORE)), sam10)
    }
    ...
    colnames(samp_vec_mat) <- c("VARIANT", "gene_symbol", 
                                ...
                                "coh_MAF_cont", "coh_MAF", "GERP_score", Ex_samp_id)
    ...
    ##compute maf SKAT takes care of the AFs internally by assigning higher weights to rare variants ; weights.beta
      ...
      gene_mat_comb <- as.numeric(as.character(samp_vec_mat[,7]))
...
saveRDS(DT_skat_snv_str_pc123, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/Syn/Exome_skat_filt3_nCH_C5eqC4_nonmds_iskrisc_05Sept_rect_ASP_syn_GERP.rds", compress = T)
