...
Exome_pc123_srt_SKAT <- lapply(df_skat, function(x) x[order(x$pval_SKATbin, decreasing = F),])
Exome_pc123_srt_SKAT <- lapply(Exome_pc123_srt_SKAT, function(x){colnames(x)[1] <- c("symbol"); return(x)})
Exome_pc123_srt_SKAT_case_enr_nCH <- readRDS("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/Exome_pc123_SKAT_Enriched_combset2020_ver4.rds")
##Add p-value
...
##use SKATO output; use SKATbin as earlier(from simulation expt)
...
fil_tab_noCH <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/all_isksmgrb_combset2020_variants_filt_all_fields.tsv", sep = "\t", header = T, stringsAsFactors = F)
pc123_gender_tab_top1000_VAR <- fil_tab_noCH[fil_tab_noCH$gene_symbol %in% pc123_gender_tab_top1000$gene & fil_tab_noCH$comb_score >= 3 & fil_tab_noCH$VAF >= 0.35,]
...
write.table(pc123_gender_tab_top1000_VAR, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/Exome_Enriched_pc_gender_controlled_top_geneVARIANTS_comset2020_pval01.tsv", sep = "\t", row.names = F, quote = F)
...
write.table(ppi_res_fil_final, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/ppi_res_fil_final_SKATbin.tsv", sep = "\t", row.names = F, quote = F)
...
#ppi_res_fil_final <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set#_2020/ISKS/ppi_res_fil_final.tsv", sep = "\t", header = T)
ppi_res_fil_final <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/ppi_res_fil_final_SKATbin.tsv", sep = "\t", header = T)
...
para_fisher_weighted_new <- function(gene_sym){
  ...
  ##simple Fisher test
  #inp <- c(deg_union, deg_biog, tot_union - deg_union, tot_biog - deg_biog)
  inp <- c(ppi_res[,1], 1661 - ppi_res[,1] , ppi_res[,2], 3209 - ppi_res[,2])
...
write.table(ppi_res_fil_final, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/ppi_res_fil_final_SKATbin_wt_fisher.tsv", sep = "\t", row.names = F, quote = F)

#write.table(ppi_res_fil_final, #"~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/ppi_res_fil_final_wt_#fisher.tsv", sep = "\t", row.names = F, quote = F)
...
ppi_res_fil_final <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/ppi_res_fil_final.tsv", sep = "\t", header = T, stringsAsFactors = F)
...
saveRDS(goNet_enrich_res, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/goNet_enrich_res_isks_combset2020.rds", compress = T)

goNet_enrich_res <- readRDS("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/goNet_enrich_res_isks_combset2020.rds")
...
fil_graph_final_comb_GO <- fil_graph_final_comb_GO[,c(1:10,13:15,17:20,22,25:31)]
...
file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/ISKS_combset2020_final_PPI_comb_GO.tsv", sep = "\t", row.names = F, quote = F)
