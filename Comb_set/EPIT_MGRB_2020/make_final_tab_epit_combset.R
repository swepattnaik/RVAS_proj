...
case_call_auto <- paste(apply(as.data.frame(table(as.character(fil_db_genes[fil_db_genes$is_case == 1,]$auto_call))) , 1 , paste , collapse = ":" ), collapse = ",")
    ...
    res[[i]] <- cbind.data.frame("EPIT" = isks, "Control" = control, "gene" = gene_sym[i], 
...
saveRDS(Exome_pc123_srt_SKAT_case_enr_nCH, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/Exome_pc123_SKAT_Enriched_EPIT_2020.rds", compress = T)
