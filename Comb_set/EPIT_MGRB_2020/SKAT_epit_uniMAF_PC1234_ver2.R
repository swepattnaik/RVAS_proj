...
fil_tab <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/all_epit_mgrb_combset2020_variants_filt_all_fields.tsv",
                      sep = "\t", stringsAsFactors = F, header = T)
...
#    skat_pvals_age_sex_pc12 <- list()
    ...
    for(i in 1:length(genes)){
      ...
      if(dim(ftemp_tab) == 0 ){
       ...
       #   sam_gene_gt <- ftemp_tab[ftemp_tab$VARIANT %in% ftemp_tab_var_id[m],][,c(1:3,9,125:126)]
          sam_gene_gt <- unique(sam_gene_gt)
          ##remove variants that have discrepancy between vep_codon and ref-alt
          ##if(dim(sam_gene_gt) > 1){conditions here; more than one vep_consequence; must be SNV(variant type)}
          #sapply(str_extract_all(x, "\\b[A-Z]+\\b"), paste, collapse= ' ')
          #test_str <- c("tgG/tgT", "tgG/tgT", "tgG/tgA")
          # library(stringr)
          ##Not required, snv variant ref-alt alleles can represent both indels and substitutions.
         # `%nin%` = Negate(`%in%`)
          #if(dim(sam_gene_gt)[1] > 1 & length(unique(sam_gene_gt$vep_consequence)) > 1 & sam_gene_gt$vep_consequence %nin% c("frameshift_variant")){
         ...
         # if(dim(sam_gene_gt)[1] > 1 & length(unique(sam_gene_gt$vep_consequence)) > 1 & length(unique(sam_gene_gt$comb_score)) > 1){
            ...
            for(k in 1:length(vep_con)){
              ...
              maf_vec_case <- sum(dim(sam_gene_gt_con[!grepl("^[ABZ]",sam_gene_gt_con$SAMPLE),])[1])/(2*920)
              ##genotype matrix  
        ...
        #  sam_gene_gt <- sam_gene_gt[which(vep_cod %in% ref_alt),]
          ...
          ##compute cohort specific MAF
           maf_vec_cont <- sum(dim(sam_gene_gt[grepl("^[ABZ]",sam_gene_gt$SAMPLE),])[1])/(2*3209)
           maf_vec_case <- sum(dim(sam_gene_gt[!grepl("^[ABZ]",sam_gene_gt$SAMPLE),])[1])/(2*920)
   ...
   #                               "coh_MAF_cont", Ex_samp_id)
      ##removed "vep_consequence"
  #    colnames(samp_vec_mat) <- c("VARIANT", "gene_symbol", "auto_call", "comb_score", "coh_MAF_case",
  #                                "coh_MAF_cont", Ex_samp_id)
     ##Intracohort filter  : equivalent to ~ 3/920*2 (920 pass QC2) for case ; ~ 5/3209*2 for control
  ...
  #                                    as.numeric(as.character(samp_vec_mat$coh_MAF_cont)) <= 0.001,]
       samp_vec_mat <- samp_vec_mat[!(as.numeric(as.character(samp_vec_mat$coh_MAF_case)) >= 0.00163 | 
                                      as.numeric(as.character(samp_vec_mat$coh_MAF_cont)) >= 0.00163),]
    #  samp_vec_mat <- samp_vec_mat[as.numeric(as.character(samp_vec_mat$coh_MAF_cont)) <= 0.001,]
      ...
      ##compute maf SKAT takes care of the AFs internally by assigning higher weights to rare variants 
        ...
        ##gender + pc1 + pc2
        skat_pvals_sex_pc12[[i]] <- SKAT_run(geno_mat = gene_mat, gene = genes[i], x=p_Data_noCH[,c(53,19:22)], p_vec, cust_weight = gene_mat_comb, rho = 1)
   ...
   ##SKAT ERA added along with changes to SKATO; now SKATBinary is used for SKATO and SKAT_ERA
    ...
    #  saveRDS(DT_skat_snv_str_pc123, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/Exome_skat_result_epit_combset2020.rds", compress = T)
    saveRDS(DT_skat_snv_str_pc123, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/Exome_skat_result_epit_combset2020_uni_MAF_PC1234_ver2.rds", compress = T)
