...
mgrb_tsv <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/MGRB/all_shards_MGRB_combset2020.tsv", sep = "\t",
                       header = T, stringsAsFactors = F)
Ex_tab <- rbind.data.frame(epit_tsv, mgrb_tsv)
rm(epit_tsv, mgrb_tsv)
...
##986 EPIT(920 pass QC2), 3209 MGRB
...
for(j in 1:dim(var_maf_df)[1]){
  ...
  MAF_case <- sum(sam_var_vec)/(2*920)
...
Ex_tab_noCH_filt3_fin_filt$MAF_case <- ifelse((is.na(Ex_tab_noCH_filt3_fin_filt$MAF_case) 
                                              ...
                                              1/(2*920), Ex_tab_noCH_filt3_fin_filt$MAF_case)
...
write.table(Ex_tab_noCH_filt3_fin_filt, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/all_epit_mgrb_combset2020_variants_filt_all_fields_risk_factor.tsv",
            row.names = F, quote = F, sep = "\t")
