...
para_SKAT <- function(genes){
  ...
  if(dim(ftemp_tab) == 0 ){
    ...
    samp_vec_mat <- samp_vec_mat[!(as.numeric(as.character(samp_vec_mat$coh_MAF_case)) >= 0.0003 | 
                                     as.numeric(as.character(samp_vec_mat$coh_MAF_cont)) >= 0.0003),]
...
saveRDS(res_skat_para, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_EPIT/Exome_skat_para_result_isks_epit_mgrb_combset2020_PC1234_ver4_rect_test.rds", compress = T)
