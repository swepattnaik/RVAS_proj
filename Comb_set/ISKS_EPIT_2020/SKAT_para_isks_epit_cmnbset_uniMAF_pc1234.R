...
#  print(genes[i])
  ...
  if(dim(ftemp_tab) == 0 ){
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      ...
      if(dim(sam_gene_gt)[1] > 1 & length(unique(sam_gene_gt$vep_consequence)) > 1){
        ...
        for(k in 1:length(vep_con)){
          ...
          maf_vec_cont <- sum(dim(sam_gene_gt_con[grepl("^[ABZ]",sam_gene_gt_con$SAMPLE),])[1])/(2*920)
    ...
    ## change to whole cohort MAF filter to 3/2571*2
    ...
    samp_vec_mat <- samp_vec_mat[!(as.numeric(as.character(samp_vec_mat$coh_MAF_case)) >= 0.00116234 | 
                                     as.numeric(as.character(samp_vec_mat$coh_MAF_cont)) >= 0.00116234),]
...
saveRDS(res_skat_para, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/Exome_skat_para_result_isks_epit_combset2020_PC1234_ver4_rect_test.rds", compress = T)
