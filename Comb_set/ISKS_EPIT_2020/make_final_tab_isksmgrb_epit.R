...
get_coh_dist <- function(gene_sym, fil_db){
  ...
  for(i in 1:length(gene_sym)){
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      ...
      maf_vec_cont <- sum(ifelse(is.na(as.numeric(sam_gene_gt$SAMPLE)), 1, 0))/(5790*2)
      maf_vec_case <- sum(ifelse(!is.na(as.numeric(sam_gene_gt$SAMPLE)) | 
                                   grepl("^CR|^LK", as.character(sam_gene_gt$SAMPLE)), 1, 0))/(5790*2)
     ...
     # print(as.numeric(as.character(maf_vec_case)))
      ...
      if(!(as.numeric(as.character(maf_vec_cont)) >= 0.0003 | 
           as.numeric(as.character(maf_vec_case)) >= 0.0003)){
...
Exome_skat <- readRDS("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_EPIT/Exome_skat_para_result_isks_epit_mgrb_combset2020_PC1234_ver4_rect_test.rds")
...
saveRDS(Exome_pc123_srt_SKAT_case_enr_nCH, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_EPIT/Exome_isks_epit_mgrb_pc1234_SKAT_Enriched_combset2020_ver4.rds", compress = T)
