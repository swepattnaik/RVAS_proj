...
# 
# gender_PC123_cont_df_cliq <- gender_PC123_cont_df[as.character(gender_PC123_cont_df$gene) %in% names(V(cpx_graph)),]
# gender_PC123_cont_df_cliq$cliq_deg <- degree(cpx_graph)

##clique size4

te4 <- cliques(uniongraph, min=4) ##use for final list
##use te1 for network enrichment
cpx <- names(unlist(te4))
cpx_genes <- unique(cpx)
cpx_np_all <- unique(can_net1[as.character(can_net1$from) %in% cpx 
                              & as.character(can_net1$to) %in% cpx, ])
cpx_graph <- igraph::graph.data.frame(cpx_np_all, directed = F)

gender_PC123_cont_df_4cliq <- gender_PC123_cont_df[as.character(gender_PC123_cont_df$gene) %in% names(V(cpx_graph)),]
gender_PC123_cont_df_4cliq$cliq_4deg <- degree(cpx_graph)
...
get_deg_rand_cliq <- function(gene_list, gene_name, cliq_size){
  can_net_graph <- igraph::graph.data.frame(can_net, directed = F)
  can_net_graph1 <- igraph::simplify(can_net_graph, remove.loops=T, remove.multiple = T)
  can_net1 <- igraph::as_data_frame(can_net_graph1, what = "edges")
  prot_np_all <- unique(can_net1[as.character(can_net1$from) %in% gene_list 
  ...
  if(dim(prot_np_all[prot_np_all$from %in% gene_name | prot_np_all$to %in% gene_name,])[1] != 0 ){
    uniongraph <- igraph::graph.data.frame(prot_np_all, directed = F)
    te1_rand <- cliques(uniongraph, min=cliq_size) 
    cpx_rand <- names(unlist(te1_rand))
    cpx_genes_rand <- unique(cpx_rand)
    cpx_np_all_rand <- unique(can_net1[as.character(can_net1$from) %in% cpx_rand 
                                  & as.character(can_net1$to) %in% cpx_rand, ])
    cpx_graph_rand <- igraph::graph.data.frame(cpx_np_all_rand, directed = F)
    if(length(grep(gene_name, names(V(cpx_graph_rand)))) > 0){
    return(igraph::degree(cpx_graph_rand, gene_name, mode="all"))
    }
    else{return(0)}
  }
  else{
...
emp_pval_gene_cliq <- function(gene_sym, cliq_size){
  
  gene_add <- lapply(gene_rand, function(x)c(gene_sym, x))
  gene_add_degree <- lapply(gene_add, function(x)get_deg_rand_cliq(x, gene_sym, cliq_size))
 # plot(hist(unlist(gene_add_degree)))
#  a <- gender_PC123_cont_df_cliq[gender_PC123_cont_df_cliq$gene %in% gene_sym,]$cliq_deg
  a <- gender_PC123_cont_df_4cliq[gender_PC123_cont_df_4cliq$gene %in% gene_sym,]$cliq_4deg
  s <- sd(unlist(gene_add_degree), na.rm = T)
  n <- length(gene_rand[[1]])
  xbar <- mean(unlist(gene_add_degree))
  t <- (xbar-a)/(s/sqrt(n))
...
##cliq_size = 3
# emp_pval <- numeric()
# for (i in 1:dim(gender_PC123_cont_df_cliq)[1]){
# 
#   print(as.character(gender_PC123_cont_df_cliq$gene[i]))
# emp_pval[i] <- emp_pval_gene_cliq(as.character(gender_PC123_cont_df_cliq$gene[i]), cliq_size = 3)
# }
# 
# gender_PC123_cont_df_cliq$emp_pval <- emp_pval
# 
# 
# write.table(gender_PC123_cont_df_cliq, 
#             "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/emp_pval/gender_PC123_cont_df_emp_pval_cliq.tsv",
#             row.names = F, quote = F, sep = "\t")

##cliq_size = 4
emp_pval4 <- numeric()
for (i in 1:dim(gender_PC123_cont_df_4cliq)[1]){
  #for (i in 1:6){
  print(as.character(gender_PC123_cont_df_4cliq$gene[i]))
  emp_pval4[i] <- emp_pval_gene_cliq(as.character(gender_PC123_cont_df_4cliq$gene[i]), cliq_size = 4)
  #  print(emp_pval_gene_cliq(as.character(gender_PC123_cont_df_cliq$gene[i])))
}

gender_PC123_cont_df_4cliq$emp_pval4 <- emp_pval4


write.table(gender_PC123_cont_df_4cliq, 
            "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/emp_pval/gender_PC123_cont_df_emp_pval_4cliq.tsv",
            row.names = F, quote = F, sep = "\t")
