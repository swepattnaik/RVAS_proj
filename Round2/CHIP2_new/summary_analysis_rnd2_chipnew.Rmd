...
Exome_pc123_srt_SKAT <- lapply(df_skat, function(x) x[order(x$pval_SKATbin, decreasing = F),])
Exome_pc123_srt_SKAT <- lapply(Exome_pc123_srt_SKAT, function(x){colnames(x)[1] <- c("symbol"); return(x)})
Exome_pc123_srt_SKAT_case_enr_nCH <- readRDS("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/discovery_set/round2/CHIP_new/Exome_pc123_SKAT_Enriched_08Oct_rect_ASP_cmaf_new_score_chip_new.rds")
##Add p-value
...
fil_tab_noCH <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/discovery_set/Disc_set_variants_filt_all.tsv", sep = "\t", header = T, stringsAsFactors = F)
...
write.table(pc123_gender_tab_top1000_VAR, "/home/shu/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/discovery_set/round2/CHIP_new/Exome_Enriched_pc_gender_controlled_top_geneVARIANTS_CHIP_new_score.tsv", sep = "\t", row.names = F, quote = F)
...
#test function with slight changes
# res <- list()
# for(i in 1:dim(gender_PC123_cont_df)[1]){
#  # print(as.character(gender_PC123_cont_df$gene[i]))
#   res[[i]] <- para_fisher_fun_new(gender_PC123_cont_df[i,])
# }

write.table(ppi_res_fil_final, "/home/shu/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/discovery_set/round2/CHIP_new/Exome_Enriched_pc_gender_controlled_top_gene_PPI_CHIP_new_score.tsv", sep = "\t", row.names = F, quote = F)
...
saveRDS(goNet_enrich_res, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/discovery_set/round2/goNet_enrich_res_sep05_rect_ASP_01pval_new_score_rnd2.rds", compress = T)

goNet_enrich_res <- readRDS("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/discovery_set/round2/goNet_enrich_res_sep05_rect_ASP_01pval_new_score_rnd2.rds")
...
write.table(fil_graph_final_comb_GO, 
file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/discovery_set/round2/CHIP_new/Discset_filt_rect_ASP_graph_final_PPI_comb_GO_cmaf_new_score_str_CHIP.tsv", sep = "\t", row.names = F, quote = F)
