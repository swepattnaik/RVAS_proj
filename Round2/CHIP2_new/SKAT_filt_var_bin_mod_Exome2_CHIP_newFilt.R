...
##Experiments
#DT_exome_clean3_var.R was used to generate Disc_set_variants_filt_all.tsv
...
fil_tab <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/discovery_set/Disc_set_variants_filt_all.tsv",
...
#    skat_pvals_age_sex_pc12 <- list()
   ...
   ##change filter based on DT discussion: VAF >= 0.35, comb_score >= 3
      ...
      if(dim(ftemp_tab)[1] == 0 ){
        ...
        for(m in 1:length(ftemp_tab_var_id)){
          sam_gene_gt <- ftemp_tab[ftemp_tab$VARIANT %in% ftemp_tab_var_id[m],][,c(1:3,9:10,38:39)]
     ...
     ##Intracohort filter  : equivalent to ~ 3/1108*2 for case ; ~ 3/2172*2
  ...
  #                                    as.numeric(as.character(samp_vec_mat$coh_MAF_cont)) <= 0.001,]
       samp_vec_mat <- samp_vec_mat[!(as.numeric(as.character(samp_vec_mat$coh_MAF_case)) >= 0.0015 | 
                                      as.numeric(as.character(samp_vec_mat$coh_MAF_cont)) >= 0.00069),]
   ...
   # saveRDS(DT_skat_snv_str_pc123, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/Exome_skat_wsing_load123_noCH_C5eqC4_nonmds_gt_isksrisc_sept05_rect_ASP_cmaf_new_score.rds", compress = T)
    saveRDS(DT_skat_snv_str_pc123, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/discovery_set/round2/CHIP_new/Exome_skat_wsing_load123_noCH_new.rds", 
            compress = T)