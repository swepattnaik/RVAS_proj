---
title: "Summary_exome_analysis_splice"
...
Exome_pc123_srt_SKAT <- lapply(df_skat, function(x) x[order(x$pval_SKATbin, decreasing = F),])
Exome_pc123_srt_SKAT <- lapply(Exome_pc123_srt_SKAT, function(x){colnames(x)[1] <- c("symbol"); return(x)})
Exome_pc123_srt_SKAT_case_enr_nCH <- readRDS("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/Exome_pc123_srt_SKAT_case_enr_nCH_iskrisc_05Sept_rect_splice_ASP_new_score.rds")
##Add p-value
...
#write.table(pc123_gender_tab_top1000_VAR, "/home/shu/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/Exome_Enriched_pc_gender_controlled_top_geneVARIANTS_sep05_ASP_splice_cmaf01_ns.tsv", sep = "\t", row.names = F, quote = F)
write.table(pc123_gender_tab_top1000_VAR, "/home/shu/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/Exome_Enriched_pc_gender_controlled_top_geneVARIANTS_sep05_ASP_splice_cmaf02_ns.tsv",sep = "\t", row.names = F, quote = F)
...
# gender_PC123_mapped <- string_db$map( gender_PC123_cont_df, "gene", removeUnmappedRows = TRUE )
...
net_new_t_list <- network(net_mat_t_list, directed = FALSE)
network.vertex.names(net_new_t_list) <- V(cpx_graph)$name
gnet_string <- ggnet2(net_new_t_list, alpha = 0.75, edge.alpha = 0.5, label = TRUE, label.size = 3,  mode = "kamadakawai") + theme(legend.position = "bottom") + theme(legend.title = element_blank())
...
saveRDS(goNet_enrich_res, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/goNet_enrich_res_sep05_rect_splice_ASP_01pval_ns.rds", compress = T)
...
fil_graph_final_comb_GO <- fil_graph_final_comb_GO[,c(1:10,14:15,17:20,22,25:34)]
inp_pval <- fil_graph_final_comb_GO[,c(11,17)]
fil_graph_final_comb_GO$comb_pval <- apply(inp_pval, 1, comb_fisher)
#write.table(fil_graph_final_comb_GO, 
#file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/Sep05_rect_Splice_ASP_graph_final_PPI_comb_GO_cmaf_ns.tsv", sep = "\t", row.names = F, quote = F)
...
# library(parallel)
# cl <- makeCluster(25)
# clusterEvalQ(cl, library(enrichR))
# clusterExport(cl, c("enrichr", "getEnrichr"), envir = .GlobalEnv)
# system.time(test_GO_para <- parApply(cl, test_GO, 1, enrichr_fun_str))
# stopCluster(cl)

saveRDS(goNet_enrich_res, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/goNet_enrich_res_sep05_rect_splice_ASP_01pval_ns_str.rds", compress = T)
...
fil_graph_final_comb_GO <- cbind.data.frame(fil_graph_final_comb_GO, fisher_res_str)
fil_graph_final_comb_GO <- fil_graph_final_comb_GO[,-c(31:32)]
fil_graph_final_comb_GO[,c(32:37)] <- goNet_enrich_res[match(fil_graph_final_comb_GO$gene, goNet_enrich_res$gene_hub),c(2:7)]
...
inp_pval <- fil_graph_final_comb_GO[,c(11,30)]
fil_graph_final_comb_GO$comb_pval_str <- apply(inp_pval, 1, comb_fisher)
write.table(fil_graph_final_comb_GO, 
file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/Sep05_rect_Splice_ASP_graph_final_PPI_comb_GO_cmaf_ns_str.tsv", sep = "\t", row.names = F, quote = F)
