...
##output of SKAT_para_isks_epit_mgrb_pc1234_apr20.R
...
#    print(i)
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      sam_gene_gt <- fil_db_genes[fil_db_genes$VARIANT %in% ftemp_tab_var_id[m],][,c(1:3,9,11,82,125:126)]
      sam_gene_gt <- unique(sam_gene_gt)
      maf_vec_cont <- sum(ifelse(is.na(as.numeric(sam_gene_gt$SAMPLE)) & 
                                   !grepl("^CR|^LK", as.character(sam_gene_gt$SAMPLE)), 1, 0))/(5771*2)
     ...
     # print(as.numeric(as.character(maf_vec_case)))
      ...
      if(!(as.numeric(as.character(maf_vec_cont)) >= 0.00026 | 
           as.numeric(as.character(maf_vec_case)) >= 0.00026)){
    ...
    mgrb <- sum(ifelse(fil_db_genes$is_case == 0, 1, 0))
    epit <- sum(ifelse(fil_db_genes$is_case == 2, 1, 0))
    control <- tot_var - isks
    case_wt <- sum(fil_db_genes[fil_db_genes$is_case == 1,]$comb_score)
    control_wt <- sum(fil_db_genes[fil_db_genes$is_case != 1,]$comb_score)
    mgrb_wt <- sum(fil_db_genes[fil_db_genes$is_case == 0,]$comb_score)
    epit_wt <- sum(fil_db_genes[fil_db_genes$is_case == 2,]$comb_score)
    ...
    mgrb_call_auto <- paste(apply(as.data.frame(table(as.character(fil_db_genes[fil_db_genes$is_case == 0,]$auto_call))) , 1 , paste , collapse = ":" ), collapse = ",")
    epit_call_auto <- paste(apply(as.data.frame(table(as.character(fil_db_genes[fil_db_genes$is_case == 2,]$auto_call))) , 1 , paste , collapse = ":" ), collapse = ",")
    ...
    mgrb_vep_var <- paste(apply(as.data.frame(table(as.character(fil_db_genes[fil_db_genes$is_case == 0,]$vep_consequence))) , 1 , paste , collapse = ":" ), collapse = ",")
    epit_vep_var <- paste(apply(as.data.frame(table(as.character(fil_db_genes[fil_db_genes$is_case == 2,]$vep_consequence))) , 1 , paste , collapse = ":" ), collapse = ",")
    res <- cbind.data.frame("ISKS" = isks, "Control" = control, "gene" = gene_sym,
                            "EPIT" = epit, "MGRB" = mgrb, "case_wt" = case_wt, "control_wt" = control_wt, 
                            "wt_diff" = wt_diff, "epit_wt" = epit_wt, "mgrb_wt" = mgrb_wt,
                                 "case_call_auto" = case_call_auto, "epit_call_auto" = epit_call_auto,
                            "mgrb_call_auto" = mgrb_call_auto, "case_vep_var" = case_vep_var, 
                            "epit_vep_var" = epit_vep_var, "mgrb_vep_var" = mgrb_vep_var,
...
saveRDS(res20, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_EPIT/round2/Exome_isks_epit_mgrb_pc1234_SKAT_Enriched_combset2020_ver4.rds", compress = T)
