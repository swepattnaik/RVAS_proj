...
##Get complex subtype from ISKS phenotype data (Mandy)
library(readxl)
...
##select only Complex genomic class

p_Data_noCH <- p_Data_noCH[p_Data_noCH$sample %nin% comb_pheno_no_comp,]
p_Data_noCH <- p_Data_noCH[p_Data_noCH$sample %nin% not_annot,]
...
#  print(genes[i])
  ...
  if(dim(ftemp_tab) == 0 ){
    ...
    ## change to entire cohort MAF filter to 3/(4870*2)##latest Apr.24
    ...
    samp_vec_mat <- samp_vec_mat[!(as.numeric(as.character(samp_vec_mat$coh_MAF_case)) >= 0.00038 | 
                                     as.numeric(as.character(samp_vec_mat$coh_MAF_cont)) >= 0.00038),]
...
saveRDS(res_skat_para, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/Exome_skat_para_result_isks_combset2020_uni_MAF_PC1234_ver4_complex.rds", compress = T)
