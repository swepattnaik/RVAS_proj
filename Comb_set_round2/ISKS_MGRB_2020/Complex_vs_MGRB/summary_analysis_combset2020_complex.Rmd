...
Exome_pc123_srt_SKAT <- lapply(df_skat, function(x) x[order(x$pval_SKATbin, decreasing = F),])
Exome_pc123_srt_SKAT <- lapply(Exome_pc123_srt_SKAT, function(x){colnames(x)[1] <- c("symbol"); return(x)})
Exome_pc123_srt_SKAT_case_enr_nCH <- readRDS("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/Exome_para_pc123_SKAT_Enriched_ISKS_2020_complex.rds")
##Add p-value
...
Exome_pc123_srt_SKAT_case_enr_nCH_pval <- lapply(Exome_pc123_srt_SKAT_case_enr_nCH_pval, function(x) x[x$pval_SKATbin < 0.1 & x$wt_diff > 0 & (x$ISKS_cmpx + x$MGRB) >= 2 ,])
...
write.table(pc123_gender_tab_top1000_VAR, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/Exome_Enriched_pc_gender_controlled_top_geneVARIANTS_comset2020_pval01_complex.tsv", sep = "\t", row.names = F, quote = F)
...
write.table(ppi_res_fil_final, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/ppi_res_fil_final_SKATbin_complex.tsv", sep = "\t", row.names = F, quote = F)
...
ppi_res_fil_final <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/ppi_res_fil_final_SKATbin_complex.tsv", sep = "\t", header = T)
...
para_fisher_weighted_new <- function(gene_sym){
  ...
  ##simple Fisher test
  #inp <- c(deg_union, deg_biog, tot_union - deg_union, tot_biog - deg_biog)
  inp <- c(ppi_res[,1], 832 - ppi_res[,1] , ppi_res[,2], 3205 - ppi_res[,2])
...
#  inp1 <- c(ppi_res[,4], 1661 - ppi_res[,1] , ppi_res[,5], 3209 - ppi_res[,2])
  inp1 <- c(ppi_res[,4], 832 - ppi_res[,1] , ppi_res[,5], 3205 - ppi_res[,2])
...
write.table(ppi_res_fil_final, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/ppi_res_fil_final_SKATbin_wt_fisher_complex.tsv", sep = "\t", row.names = F, quote = F)
...
ppi_res_fil_final <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/ppi_res_fil_final_SKATbin_wt_fisher_complex.tsv", sep = "\t", header = T, stringsAsFactors = F)
...
saveRDS(goNet_enrich_res, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/goNet_enrich_res_isks_combset2020_complex.rds", compress = T)
...
file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/ISKS_complex_combset2020_final_PPI_comb_GO_complex.tsv", sep = "\t", row.names = F, quote = F)
