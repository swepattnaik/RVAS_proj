...
fil_tab_noCH <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/all_isksmgrb_combset2020_variants_filt_all_fields_rm_dup.tsv",
...
val_test = function(data, phen_vec, samp_id, gene_sym, coh, seed = 3453466, B = 1000, ...)
{
  ft_df <- list()
...
rep_set <- do.call("rbind.data.frame", lapply(t1000, function(x)x[[2]]))
  ...
  write.table(disc_set, paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/Validation/",
                               names(cpx_list)[k],"_fisher.disc.tsv"), sep = "\t", quote = F, row.names = F)
  write.table(rep_set, paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/Validation/",
...
p <- ggboxplot(disc_melt, x = "variable", y = "value",
               ...
               add = "jitter")
p + stat_compare_means(method = "wilcox.test", method.args = list(alternative = "less"))

ggplot(disc_melt, aes(x=value, fill=variable)) + geom_density(alpha=0.5, position="identity")
...
p1 <- ggboxplot(rep_melt, x = "variable", y = "value",
               color = "variable", palette = "jco",
               add = "jitter")
p1 + stat_compare_means(method = "wilcox.test", method.args = list(alternative = "less"))

ggplot(rep_melt, aes(x=value, fill=variable)) + geom_density(alpha=0.5, position="identity")
