...
Ex_tab <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/TP53_nc/CPX_genes/cpx_genes_var_encode.tsv", sep = "\t",
                     header = T, stringsAsFactors = F)
Ex_tab$SAMPLE <- gsub("/nvme.*:", "", Ex_tab$SAMPLE)
...
##retain all non-coding variants
toMatch <- c("synonymous_variant", "intergenic_variant", "mature_miRNA_variant")
#"NMD_transcript_variant"
Ex_tab_filt1 <- Ex_tab_filt1[!(Ex_tab_filt1$vep_consequence %in% toMatch),]
...
Ex_tab_filt1$is_ASPC <- ifelse(Ex_tab_filt1$SAMPLE %in% ASPREE_can_samp, 1, 0)
Ex_tab_filt1$is_45up <- ifelse(Ex_tab_filt1$SAMPLE %in% can269$V1, 1, 0)


##filter out MGRB samples that have CH

#Ex_tab_noCH_filt3 <- Ex_tab_filt2[!(Ex_tab_filt2$is_CH == 0 & Ex_tab_filt2$is_MGRB == 1),]
Ex_tab_noCH_filt3 <- Ex_tab_filt1[Ex_tab_filt1$is_ASPC == 0 & Ex_tab_filt1$is_45up == 0,]
...
##Add non-coding intronic variants (3'UTR, 5'UTR, introns, promoter)
toMatch3 <- c("3_prime_UTR_variant", "5_prime_UTR_variant", "intron_variant", 
...
sco_fun5 <- function(filt3_inp){
  ...
  metric1 <-  ifelse(grepl(paste(toMatch1,collapse="|"), 
                     ...
                     ifelse(grepl(paste(toMatch2,collapse="|"), 
                            ...
                            ifelse(filt3_inp$vep_con1 %in% 
                                   ...
                                   & filt3_inp$gnomad_AF_NFE == 0, "C3", 
                                   ifelse(filt3_inp$vep_con1 %in% toMatch3 & 
                                            filt3_inp$gnomad_AF_NFE == 0, "NC", "B"))))
  ...
  metric2 <- as.numeric(ifelse(metric1 %in% "C5", 45, 
                               ifelse(metric1 %in% "C4", 45, 
                                      ifelse(metric1 %in% "C3", filt3_inp$EigenPhred, 
                                             ifelse(metric1 %in% "NC", 10 ,0)))))
...
`%nin%` = Negate(`%in%`)
Ex_tab_noCH_filt3 <- Ex_tab_noCH_filt3[as.character(Ex_tab_noCH_filt3$auto_call) %nin% "B",]



#Tag QC2 failed
##new step added from repset variant filtering in addition to changes in MGRB chip set
Ex_tab_noCH_filt3$QC2 <- ifelse(Ex_tab_noCH_filt3$SAMPLE %in% QC2_dat_fail, "fail", "pass")

#remove benigns and QC2 failed samples
##QC2 failed samples: ("1383", "AABTU", "1", "AACDY")
Ex_tab_noCH_filt3_fin_filt <- Ex_tab_noCH_filt3[Ex_tab_noCH_filt3$QC2 %in% "pass",]
...
Ex_tab_noCH_filt3_fin_filt$set <- c("regul_comb")
...
write.table(Ex_tab_noCH_filt3_fin_filt, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/reg_elt_cpx_isksmgrb_combset2020_variants_filt_noMAF_rmdup.tsv",
