...
fil_tab <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/nc_all_isksmgrb_combset2020_variants_filt_noMAF.tsv",
...
##save table
fil_tab <- fil_tab[fil_tab$auto_call %in% "NC",]
fil_tab$is_case <- ifelse(!is.na(as.numeric(as.character(fil_tab$SAMPLE))) | 
                                               grepl("^CR|^LK",as.character(fil_tab$SAMPLE)), 1, 0)
write.table(fil_tab, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/nc_all_isksmgrb_combset2020_variants_filt_all_fields_rm_dup.tsv", 
...
#genomicclass
comb_pheno_no_comp <- as.character(comb_pheno[comb_pheno$genomicclass %nin% "Simple",]$pmn)
##13 SIMPLE cases were not annotated in Mandy's data
not_annot <- Ex_samp_id[Ex_samp_id %nin% comb_pheno[comb_pheno$genomicclass %in% "Simple",]$pmn & !grepl("^[ABZ]", Ex_samp_id)]

##filter out QC fail cases
fil_tab <- fil_tab[fil_tab$SAMPLE %nin% comb_pheno_no_comp,]
fil_tab <- fil_tab[fil_tab$SAMPLE %nin% not_annot,]
fil_tab <- fil_tab[fil_tab$SAMPLE %in% Ex_samp_id,]
length(unique(fil_tab$SAMPLE))
length(unique(fil_tab[fil_tab$is_case == 1,]$SAMPLE))
...
get_coh_dist_para <- function(gene_sym, fil_db){
  ...
  if(length(ftemp_tab_var_id) == 0 ){ ##added on apr.22
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      ...
      maf_vec_cont <- sum(ifelse(is.na(as.numeric(sam_gene_gt$SAMPLE)), 1, 0))/((370 + 3205)*2)
      maf_vec_case <- sum(ifelse(!is.na(as.numeric(sam_gene_gt$SAMPLE)) | 
                                   grepl("^CR|^LK", as.character(sam_gene_gt$SAMPLE)), 1, 0))/((370 + 3205)*2)
      ##MAF filter = 5/(1661*2); change to 3/(1661*2)
      ##changed to cohort MAF threshold : 3/((370 + 3209)*2) 
  ...
  res <- cbind.data.frame("ISKS_simple" = isks, "MGRB" = control, "gene" = gene_sym, 
...
#for(k in 1:length(unique(fil_tab$gene_symbol))){
res_list <- list()
...
saveRDS(res20, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/nc_Exome_para_ISKS_SIMPLE_2020_C2.rds", compress = T)
