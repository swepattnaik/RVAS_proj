...
fil_tab <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/all_isksmgrb_combset2020_variants_filt_all_fields_rmdup_rmdeg2.tsv",
                      sep = "\t", stringsAsFactors = F, header = T)
...
#  print(genes[i])
  ...
  if(dim(ftemp_tab) == 0 ){
    ...
    samp_vec_mat <- samp_vec_mat[!(as.numeric(as.character(samp_vec_mat$coh_MAF_case)) >= 0.0004 | 
                                     as.numeric(as.character(samp_vec_mat$coh_MAF_cont)) >= 0.0004),]
...
saveRDS(res_skat_para, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS/round2/Exome_skat_para_result_isks_combset2020_uni_MAF_PC1234_ver4_deg2.rds", compress = T)
