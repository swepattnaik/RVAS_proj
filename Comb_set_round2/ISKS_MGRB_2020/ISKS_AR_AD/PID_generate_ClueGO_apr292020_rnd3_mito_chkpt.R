...
Sheltrin_comp_extn = c("ACD", "POT1", "TERF1", "TERF2", "TERF2IP", "TINF2", "ATM", 
                       ...
                       "WRN", "XRCC5", "XRCC6")
##Add mito_chk_point
gene_sub <- unique(c(top_SKAT_cgc$V1, cgc_genes$Gene.Symbol, sheltrin_complex, Telo_extension, Sheltrin_comp_extn, mito_chk_point$V1))
...
#write.table(genes, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/PID/CGC_topSKATBin300_repstress_potint_PID_unique_genes_cluego_Aug19.txt",
#            sep = "\t", row.names = F, quote = F)
write.table(genes, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/PID/CGC_topSKATBin300_repstress_potint_mito_chkpt_PID_unique_genes_cluego_Aug19.txt",
            sep = "\t", row.names = F, quote = F)
...
# genes <- unique(fil_tab_AD$gene_symbol)
# system.time(res20 <- foreach(i=1:length(genes), .errorhandling = 'remove') %dopar% 
# {gene_var_maf(genes[i], fil_tab_AD)})
# gene_maf_pass_df <- do.call("rbind.data.frame", res20)
# comb_set_filt1_AD <- comb_set_filt_AD[comb_set_filt_AD$VARIANT %in% gene_maf_pass_df$VARIANT,]
# comb_set_filt1 <- rbind.data.frame(comb_set_filt1_AD, comb_set_filt_AR)
...
#            sep = "\t", row.names = F, quote = F)
#write.table(comb_set_filt1, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/PID/CGC_topSKATBin300_repstress_potint_VARIANTS_filt_combset2020_clueGOplus_Aug19.tsv",
#            sep = "\t", row.names = F, quote = F)
write.table(comb_set_filt1, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/PID/CGC_topSKATBin300_repstress_potint_mito_chkpt_VARIANTS_filt_combset2020_clueGOplus_Aug19.tsv",
            sep = "\t", row.names = F, quote = F)
...
write.table(fil_tab_PID, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/PID/CGC_topSKATBin_ISKS_MRGB_repstress_potint_mito_chkpt_VARIANTS_filt_combset2020_clueGOplus_Aug19.tsv",
            sep = "\t", row.names = F, quote = F)
...
comb_ALL_phen[,c(68:84)] <- df_sam_filt_isks_comb[match(as.character(comb_ALL_phen$pmn), df_sam_filt_isks_comb$SAMPLE), c(1:17)]
...
write.table(comb_ALL_phen_filt[,-c(75:81)], 
            "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/PID/PID_combset2020_CGC_skatBin_repstress_potint_mito_chkpt_predNFE_clueGO_Apr292020_AD_Aug19.tsv",
            sep = "\t", row.names = F, quote = F)


##check if labelling needs to be done for all AR or only AR homozygous..
