...
get_coh_dist_para <- function(gene_sym, fil_db){
  ...
  if(length(ftemp_tab_var_id) == 0 ){ ##added on apr.22
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      ...
      maf_vec_cont <- length(grep("^[ABZ]", sam_gene_gt$SAMPLE))/((370 + 3205)*2)
  ...
  res <- cbind.data.frame("ISKS_sim" = isks, "MGRB" = control, "gene" = gene_sym, 
...
saveRDS(res20, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Simple/Exome_para_ISKS_SIMPLE_2020_minusC3.rds", compress = T)
