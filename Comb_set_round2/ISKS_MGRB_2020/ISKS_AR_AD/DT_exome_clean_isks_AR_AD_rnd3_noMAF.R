...
Ex_tab_clinvar_gene <- Ex_tab[!is.na(Ex_tab$clinvar_Gene),]
...
fs_var <- c("indel", "insertion", "sequence_alteration")
...
Ex_tab_Pathogenic_C4 <- Ex_tab_Pathogenic[Ex_tab_Pathogenic$vep_VARIANT_CLASS %in% fs_var,]
#gene_map_rescue_path <- unique(Ex_tab_Pathogenic[Ex_tab_Pathogenic$gene_symbol %in% gene_map_rescue$chk_gene_diff_sym,c(9,32)])
#cgc_skat_genes <- gene_sub[gene_sub %in% gene_map_rescue_path$clinvar_Gene]
gene_map_rescue_path_C5 <- unique(Ex_tab_Pathogenic_C5[,c(9,32)])
gene_map_rescue_path_C5 <- gene_map_rescue_path_C5[gene_map_rescue_path_C5$gene_symbol %nin% gene_map_rescue_path_C5$clinvar_Gene,]
#cgc_skat_genes_C5 <- gene_sub[gene_sub %in% gene_map_rescue_path_C5$clinvar_Gene]

gene_map_rescue_path_C4 <- unique(Ex_tab_Pathogenic_C4[,c(9,32)])
gene_map_rescue_path_C4 <- gene_map_rescue_path_C4[gene_map_rescue_path_C4$gene_symbol %nin% gene_map_rescue_path_C4$clinvar_Gene,]
#cgc_skat_genes_C4 <- gene_sub[gene_sub %in% gene_map_rescue_path_C4$clinvar_Gene]

#merge the set of rescue variants
gene_map_rescue_path <- unique(rbind.data.frame(gene_map_rescue_path_C5, gene_map_rescue_path_C4))

rescue_path_C5 <-Ex_tab_Pathogenic_C5[Ex_tab_Pathogenic_C5$gene_symbol %in% gene_map_rescue_path_C5$gene_symbol & 
                       Ex_tab_Pathogenic_C5$clinvar_Gene %in% gene_map_rescue_path_C5$clinvar_Gene,]
rescue_path_C4 <-Ex_tab_Pathogenic_C4[Ex_tab_Pathogenic_C4$gene_symbol %in% gene_map_rescue_path_C4$gene_symbol & 
                                        Ex_tab_Pathogenic_C4$clinvar_Gene %in% gene_map_rescue_path_C4$clinvar_Gene,]
rescue_path_C4C5 <- unique(rbind.data.frame(rescue_path_C4, rescue_path_C5))
rescue_path_C4C5 <- rescue_path_C4C5[rescue_path_C4C5$VAF >= 0.35 & rescue_path_C4C5$DP >= 10,]
rescue_path_C4C5 <- rescue_path_C4C5[rescue_path_C4C5$vep_consequence %nin% "intron_variant",]
clinvar_Name_ind <- ifelse(grepl("\\_\\(p", rescue_path_C4C5$clinvar_Name), 1, 0)
rescue_path_C4C5$clinvar_Name_ind <- clinvar_Name_ind
rescue_path_C4C5 <- rescue_path_C4C5[rescue_path_C4C5$clinvar_Name_ind == 1,]
...
rescue_path_C4C5$CANONICAL_GENE_LENGTH_IN_AMIMO_ACIDS <- prot_len[match(rescue_path_C4C5$clinvar_Gene, prot_len$gene), 2]
...
rescue_path_C4C5$LONGEST_GENE_LENGTH_IN_AMINO_ACIDS <- prot_len[match(rescue_path_C4C5$clinvar_Gene, prot_len$gene), 3]
...
rescue_path_C4C5$gene_symbol <- rescue_path_C4C5$clinvar_Gene
...
clig_gene_len <- prot_len[match(Ex_tab2$clinvar_Gene, prot_len$gene), 2]
true_id <- ifelse(Ex_tab2$gene_symbol %in% gene_true, 1, 0)
Ex_tab2$CANONICAL_GENE_LENGTH_IN_AMIMO_ACIDS <- ifelse(true_id == 1,canonical_gene_len, clig_gene_len)

longest_gene_len <- prot_len[match(Ex_tab2$clinvar_Gene, prot_len$gene), 3]

Ex_tab2$LONGEST_GENE_LENGTH_IN_AMINO_ACIDS <- ifelse(true_id == 1,Ex_tab2$LONGEST_GENE_LENGTH_IN_AMINO_ACIDS, longest_gene_len)
Ex_tab2$gene_symbol <- ifelse(true_id == 1, Ex_tab2$gene_symbol, Ex_tab2$clinvar_Gene)
...
#clinvar_Name_ind == 2
Ex_tab3 <- Ex_tab1[Ex_tab1$clinvar_Name_ind == 2,]
...
Ex_tab_comb <- rbind.data.frame(Ex_tab2, Ex_tab3)
# remove mis-annotated variants
Ex_tab_comb <- Ex_tab_comb[Ex_tab_comb$filt_tab %nin% rescue_path_C4C5$filt_tab,] 
##Add reformatted/rescued variants
Ex_tab_comb <- rbind.data.frame(Ex_tab_comb, rescue_path_C4C5)
Ex_tab_comb <- Ex_tab2[,-120] #remove filt_tab

#Ex_tab_filt1 <- Ex_tab1[Ex_tab1$gnomad_AF_NFE <= 0.0001, ] ##check this; loses some variants from Aug12 on Sept05 run(don't use)
rm(Ex_tab)
##remove SAMPLE CR57
Ex_tab_filt1 <- Ex_tab_comb[!(Ex_tab_comb$SAMPLE %in% "CR57"),]
...
write.table(Ex_tab_noCH_filt3_fin_filt, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/all_isks_combset2020_variants_AR_AD_all_fields_clingene_rnd3.tsv",
            row.names = F, quote = F, sep = "\t")
...
# ##For DT 
# Ex_tab_noCH_filt3_fin_filt <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/all_isks_combset2020_variants_AR_AD_all_fields_rnd3.tsv", sep = "\t", header = T, stringsAsFactors = F)
...
# 
# #write.table(Ex_tab_noCH_filt3_fin_filt_C4C5, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/AD_all_isks_combset2020_C4C5_gnomad01.tsv",
# #            row.names = F, quote = F, sep = "\t")
# write.table(Ex_tab_noCH_filt3_fin_filt_C3, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/AD_all_isks_combset2020_C3_gnomad00.tsv",
#             row.names = F, quote = F, sep = "\t")
# write.table(Ex_tab_noCH_filt3_fin_filt_C4C5_NFE0002, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/AD_all_isks_combset2020_C4C5_gnomad01_NFE0002_swe001_cohMAF001.tsv",
#                         row.names = F, quote = F, sep = "\t")
# write.table(Ex_tab_noCH_filt3_fin_filt_C4C5_NFE0005, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/AD_all_isks_combset2020_C4C5_gnomad01_NFE0005_swe001_cohMAF001.tsv",
#             row.names = F, quote = F, sep = "\t")
# ##AR
...
# #            row.names = F, quote = F, sep = "\t")
# write.table(Ex_tab_noCH_filt3_fin_filt_C4C5_AR_GT1_GT2nfe0002, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/AR_all_isks_combset2020_C4C5_PID_genes_gnomad05_gt2_gt1_swe001_cohMAF001_nfe0002.tsv",
#                         row.names = F, quote = F, sep = "\t")
# write.table(Ex_tab_noCH_filt3_fin_filt_C4C5_AR_GT1_GT2nfe0005, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/AR_all_isks_combset2020_C4C5_PID_genes_gnomad05_gt2_gt1_swe001_cohMAF001_nfe0005.tsv",
#             row.names = F, quote = F, sep = "\t")
# 
...
# write.table(ARnfe0002_cpd_het, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/AR_isks_compd_het_C4C5_PID_genes_gnomad05_gt2_gt1_swe001_cohMAF001_nfe0002.tsv",
#             row.names = F, quote = F, sep = "\t")
# 
...
# 
# write.table(ARnfe0005_cpd_het, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/AR_isks_compd_het_C4C5_PID_genes_gnomad05_gt2_gt1_swe001_cohMAF001_nfe0005.tsv",
#             row.names = F, quote = F, sep = "\t")
