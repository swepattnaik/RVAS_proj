...
##remove AMR samples
AMR_samp = as.character(p_Data_noCH[p_Data_noCH$pred.superPop.AMR > 0.6,]$rect_sam)
p_Data_noCH = p_Data_noCH[as.character(p_Data_noCH$rect_sam) %nin% AMR_samp,]
Ex_samp_id = Ex_samp_id[Ex_samp_id %nin% AMR_samp]
...
para_SKAT <- function(genes){
  ...
  if(dim(ftemp_tab) == 0 ){
    ...
    ##After removal of AMR and AFR entire cohort MAF filter to 3/(4743*2)##latest Jan.7.2022
    samp_vec_mat <- samp_vec_mat[!(as.numeric(as.character(samp_vec_mat$coh_MAF_case)) >= 0.00032 | 
                                     as.numeric(as.character(samp_vec_mat$coh_MAF_cont)) >= 0.00032),]
...
saveRDS(res20, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/review_2/SKAT/Exome_skat_para_result_isks_totex_count_PC1234_subPC1_EMMAX_noAMR.rds", compress = T)
##sanity checks
