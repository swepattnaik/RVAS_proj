...
score_rect_eur_3_no1000G$superPopulation = ifelse(score_rect_eur_3_no1000G$superPopulation %in% "MGRB", "controls", "sarcoma")
...
make_3Dplot_eurpop3 <- function(df_inp, theta=0, phi=0){
  ggplot(df_inp, aes(x=PC3, y=PC2, z=PC1, colour=pred.eurPop.class.3)) +
    axes_3D(theta=theta, phi=phi) +
    stat_3D(theta=theta, phi=phi, geom="point") +
    labs_3D(theta=theta, phi=phi, 
            labs=c("PC3", "PC2", "PC1"), 
...
make_3Dplot_coh <- function(df_inp, theta=0, phi=0){
  ggplot(df_inp, aes(x=PC3, y=PC2, z=PC1, colour=superPopulation)) +
    axes_3D(theta=theta, phi=phi) +
    stat_3D(theta=theta, phi=phi, geom="point") +
    labs_3D(theta=theta, phi=phi, 
            labs=c("PC3", "PC2", "PC1"), 
...
eurpop3D_12 = make_3Dplot_eurpop3(score_rect_eur_3_no1000G_sub_PC1, theta=-120, phi=20)

eurcoh3D_12 = make_3Dplot_coh(score_rect_eur_3_no1000G_sub_PC1, theta=-120, phi=20)
...
library(ggrepel)
set.seed(678456)
tsne.norm = Rtsne(score_rect_eur_3_no1000G[,19:22], pca = FALSE)
info.norm = tibble(truth = factor(score_rect_eur_3_no1000G$pred.eurPop.class.3), cohort = factor(score_rect_eur_3_no1000G$superPopulation))
info.norm %<>% mutate(tsne1 = tsne.norm$Y[, 1], tsne2 = tsne.norm$Y[,2])

tsne_eur3_pop = ggplot(info.norm, aes(x = tsne1, y = tsne2, colour = truth)) + 
...
##clustering to identify IBS cluster with skewed case control ratio
