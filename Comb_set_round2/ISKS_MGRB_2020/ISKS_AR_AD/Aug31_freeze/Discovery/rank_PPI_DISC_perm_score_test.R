...
ISKS_MGRB_2020_rnd3 <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/SKAT_disc/all_DISC_isksmgrb_skatinp_combset2020_clin_C3C4C5_NFE0002_AD_rm_dup_freeze.tsv",
                                  sep = "\t", header = T, stringsAsFactors = F)

ppi_res_fil_final <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/SKAT_disc/ppi_res_fil_final_SKATbin_comb_str_biog_Aug31_noOlfac_230920.tsv", 
                                sep = "\t", header = T, stringsAsFactors = F)
...
set.seed(473839)
##fix degree zero
ppi_res_fil_final[ppi_res_fil_final$degree == 0,]$PPI_p_val_wt <- sample(seq(0.6, 0.9, 0.0001), size = 580, replace = F)
set.seed(473839)
ppi_res_fil_final[ppi_res_fil_final$degree_str == 0,]$PPI_p_val_wt_str <- sample(seq(0.6, 0.9, 0.0001), size = 287, replace = F)
set.seed(473839)
ppi_res_fil_final[ppi_res_fil_final$degree_str_biog == 0,]$PPI_p_val_wt_str_biog <- sample(seq(0.6, 0.9, 0.0001), size = 192, replace = F)
...
ppi_res_fil_final[,51:56] <- res20_df_comb[match(ppi_res_fil_final$gene, res20_df_comb$gene),c(4:5,10:11,16:17)]
...
write.table(ppi_res_fil_final[,c(1:12,14,17:20,22,25:28,31:34,37:65)], "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/SKAT_disc/scrambled_null_ppi_fisher_res_fil_final_Aug31_230920.tsv",
            sep = "\t", row.names = F, quote = F)
