...
##subtract 200 cases used in augmented repset
repset_763 <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/Comb_TCGA_BERG_NoSarc/augmented_sarc/all_tcga_nosarc_berg_isks200_mgrb.tsv",
                         sep = "\t", header = T, stringsAsFactors = F)
repset_763_isks <- unique(repset_763[repset_763$set %in% "ISKS_AR_AD",]$SAMPLE)
Ex_samp_id <- Ex_samp_id[Ex_samp_id %nin% repset_763_isks]
...
##save table
write.table(fil_tab, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/SKAT_disc/all_DISC_isksmgrb_skatinp_combset2020_clin_C3C4C5_NFE0002_AD_rm_dup_freeze.tsv", 
...
#  print(genes[i])
  ...
  if(dim(ftemp_tab) == 0 ){
    ...
    ## change to entire cohort MAF filter to 3/(4649*2)##latest Sep.23 for discovery set (same threshold(0.00035) can persist)
...
saveRDS(res_skat_para, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/SKAT_disc/Exome_skat_para_DISC_isks_combset2020_uni_MAF_PC1234_clinrect_Aug31_230920.rds", compress = T)
