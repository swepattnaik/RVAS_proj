...
para_SKAT <- function(genes){
  ...
  if(dim(ftemp_tab) == 0 ){
    ...
    ##compute maf SKAT takes care of the AFs internally by assigning higher weights to rare variants 
      ...
      skat_pvals_sex_pc12 <- SKAT_run(geno_mat = gene_mat, gene = genes, x=p_Data_noCH[,c(40:41,19:22)], p_vec, cust_weight = gene_mat_comb, rho = 1)
...
#saveRDS(res_skat_para, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/review_doc/SKAT/Exome_skat_para_result_isks_no1000G_PC1234_ver4_clinrect_Aug31.rds", compress = T)
saveRDS(res_skat_para, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/review_doc/SKAT/Exome_skat_para_result_isks_no1000G_PC1234_ver4_totex.rds", compress = T)
