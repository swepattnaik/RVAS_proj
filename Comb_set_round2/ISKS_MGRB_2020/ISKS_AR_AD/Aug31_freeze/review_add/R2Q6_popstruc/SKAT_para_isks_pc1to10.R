...
para_SKAT <- function(genes){
  ...
  if(dim(ftemp_tab) == 0 ){
    ...
    ##compute maf SKAT takes care of the AFs internally by assigning higher weights to rare variants 
      ...
      ##gender + pc1 to pc10
      skat_pvals_sex_pc12 <- SKAT_run(geno_mat = gene_mat, gene = genes, x=p_Data_noCH[,c(54,19:28)], p_vec, cust_weight = gene_mat_comb, rho = 1)
...
saveRDS(res_skat_para, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/review_doc/SKAT/Exome_skat_para_result_isks_PC1to10_test_2021.rds", compress = T)
