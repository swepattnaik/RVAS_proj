...
for(k in 1:length(cpx_list)){
  ftemp_cpx <- fil_tab_cpx_genes[fil_tab_cpx_genes$gene_symbol %in% names(cpx_list[[k]]),]
  ...
  write.table(disc_set, paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Cliques/",
                               names(cpx_list)[k],"_fisher.disc_null.tsv"), sep = "\t", quote = F, row.names = F)
  write.table(rep_set, paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Cliques/",
...
# library(reshape2)
# 
# plot_val_data <- function(cmpx_name,plot_type = c("box", "dens"), base_dist){
#   dens_plot <- list()
#   box_plots <- list()
#   df1_disc <- read.delim(paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Cliques/",cmpx_name,"_fisher.disc_null.tsv"),
#                          sep = "\t", header = T, stringsAsFactors = F )
#   df1_rep <- read.delim(paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Cliques/",cmpx_name,"_fisher.rep_null.tsv"),
#                         sep = "\t", header = T, stringsAsFactors = F )
# 
#   disc_melt <- melt(df1_disc[,2:4])
#   rep_melt <- melt(df1_rep[,2:4])
#   if(base_dist == "control"){
#     disc_melt <- disc_melt[disc_melt$variable %nin% "null_freq",]
#     rep_melt <- rep_melt[rep_melt$variable %nin% "null_freq",]
#   }
#   else{
#     disc_melt <- disc_melt[disc_melt$variable %nin% "Controls",]
#     rep_melt <- rep_melt[rep_melt$variable %nin% "Controls",]
#   }
#  # disc_melt <- melt(df1_disc[,c(2,4)])
#  # rep_melt <- melt(df1_rep[,c(2,4)])
# 
#   if(plot_type == "box"){
...
# 
# 
# plot_OR_data <- function(cmpx_name,plot_type = c("box", "dens")){
#   dens_plot <- list()
#   box_plots <- list()
#   df1_disc <- read.delim(paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Cliques/",cmpx_name,"_fisher.disc_null.tsv"),
#                          sep = "\t", header = T, stringsAsFactors = F )
...
#                         sep = "\t", header = T, stringsAsFactors = F )
#   
#   df_comb_disc_rep <- rbind.data.frame(df1_disc[,c(8,10)], df1_rep[,c(8,10)])
#   if(plot_type == "box"){
#     p <- ggboxplot(df_comb_disc_rep, x = "Set", y = "OR_Fish",
...
#   
#   else if(plot_type == "dens"){
#     p1 <- ggplot(df_comb_disc_rep, aes(x=OR_Fish, fill=Set)) + geom_density(alpha=0.5, position="identity") +
#       ggtitle(cmpx_name)
#     
#     dens_plots <- p1
#     return(dens_plots)
...
# }
# ###########
# CI_OR_data <- function(cmpx_name, conf_interval){
#   df1_disc <- read.delim(paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Cliques/",cmpx_name,"_fisher.disc_null.tsv"),
#                          sep = "\t", header = T, stringsAsFactors = F )
#   df1_rep <- read.delim(paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Cliques/",cmpx_name,"_fisher.rep_null.tsv"),
#                         sep = "\t", header = T, stringsAsFactors = F )
# 
#  # df_comb_disc_rep <- rbind.data.frame(df1_disc[,c(8,10)], df1_rep[,c(8,10)])
#   ##t-distribution based 95 % confidence interval
#   mean_ci_fun <- function(df){
#     vec_mean <- mean(df$OR_Fish)
#     s <- sd(df$OR_Fish)
#     n <- length(df$OR_Fish)
#     q <- conf_interval + (1 - conf_interval)/2
#     error <- qt(q,df=n-1)*s/sqrt(n)
#     left <- vec_mean - error
#     right <- vec_mean + error
#     ret_df <- cbind.data.frame("cmpx_name" = cmpx_name, "mean_OR" = vec_mean, "lower.CI" = left, "upper.CI" = right)
#     return(ret_df)
#   }
# 
#   return(list(mean_ci_fun(df1_disc), mean_ci_fun(df1_rep)))
# 
# }
# 
# CI_meta_OR_data <- function(cmpx_name, conf_interval){
#   df1_disc <- read.delim(paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Cliques/",cmpx_name,"_fisher.disc_null.tsv"),
#                          sep = "\t", header = T, stringsAsFactors = F )
#   df1_rep <- read.delim(paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Cliques/",cmpx_name,"_fisher.rep_null.tsv"),
#                         sep = "\t", header = T, stringsAsFactors = F )
#   comb_OR_fun <- function(df){
#     df$meta_OR_Fish <- log(df$OR_Fish)
#     df$meta_lCI <- log(df$CI_lower)
#     df$meta_uCI <- log(df$CI_upper)
#     df$meta_SE <- (df$meta_uCI - df$meta_lCI)/3.92 ##for 95% CI
#     fixed_mod <- metagen(meta_OR_Fish, 
#             meta_SE, 
#             studlab = gene,
#             method.tau = "SJ",
#             sm = "OR",
...
# # box_plot_OR <- list()
# # for(i in 1:length(cpx_list)){
# #   box_plot_OR[[i]] <- plot_OR_data(names(cpx_list)[i], "box")
# # }
# # 
# # dens_plot_OR <- list()
# # for(i in 1:length(cpx_list)){
# #   dens_plot_OR[[i]] <- plot_OR_data(names(cpx_list)[i], "dens")
# # }
# # 
# # ######OR CI
# # 
# CI_OR <- list()
# for(i in 1:length(cpx_list)){
#   CI_OR[[i]] <- CI_OR_data(names(cpx_list)[i], 0.95)
# }
# 
# CI_disc <- lapply(CI_OR, function(x)x[[1]])
# CI_disc_df <- do.call("rbind.data.frame", CI_disc)
# CI_disc_df$set <- "Disc"
# CI_rep <- lapply(CI_OR, function(x)x[[2]])
# CI_rep_df <- do.call("rbind.data.frame", CI_rep)
# CI_rep_df$set <- "Rep"
# 
# CI_comb_disc_rep_df <- rbind.data.frame(CI_disc_df, CI_rep_df)
# CI_comb_disc_rep_df$Index <- rownames(CI_comb_disc_rep_df)
# ##Forest plot
# library(ggplot2)
# 
...
# p <- ggplot(data=CI_comb_disc_rep_df, aes(y=cmpx_name, x=log2(mean_OR), xmin=log2(lower.CI), xmax=log2(upper.CI))) + 
#   geom_point() + geom_point(data=subset(CI_comb_disc_rep_df, set=="Disc"), color="Black", size=2) + 
#   geom_errorbarh(height=.1) + scale_x_continuous(limits=c(0,5), breaks = c(0:5), name=xname) +
...
# 
# 
# p1 <- ggplot(data=CI_comb_disc_rep_df, aes(y=cmpx_name, x=mean_OR, xmin=lower.CI, xmax=upper.CI)) + 
#   geom_point() + geom_point(data=subset(CI_comb_disc_rep_df, set=="Disc"), color="Black", size=2) + 
#   geom_errorbarh(height=.1) + scale_x_continuous(limits=c(0,20), breaks = c(0:20), name=xname) +
#   geom_vline(xintercept=0, color="black", linetype="dashed", alpha=.5) + 
#   facet_grid(set~., scales= "free", space="free") + ggtitle("PPI cliques")+
#   theme_minimal() + theme(text=element_text(family="Times",size=18, color="black"))+
#   theme(panel.spacing = unit(1, "lines"))
...
# CI_OR_meta <- list()
# for(i in 1:length(cpx_list)){
#   CI_OR_meta[[i]] <- CI_meta_OR_data(names(cpx_list)[i], 0.95)
# }
# 
# CI_disc_meta <- lapply(CI_OR_meta, function(x)x[[1]])
# CI_disc_meta_df <- do.call("rbind.data.frame", CI_disc_meta)
# CI_disc_meta_df$set <- "Disc"
# CI_rep_meta <- lapply(CI_OR_meta, function(x)x[[2]])
# CI_rep_meta_df <- do.call("rbind.data.frame", CI_rep_meta)
# CI_rep_meta_df$set <- "Rep"
# 
# CI_comb_meta_df <- rbind.data.frame(CI_disc_meta_df, CI_rep_meta_df)
# CI_comb_meta_df$Index <- rownames(CI_comb_meta_df)
# 
# p <- ggplot(data=CI_comb_meta_df, aes(y=cmpx_name, x=log2(Fish_OR_meta), xmin=log2(lower.CI), xmax=log2(upper.CI))) + 
#   geom_point() + geom_point(data=subset(CI_comb_meta_df, set=="Disc"), color="Black", size=2) + 
#   geom_errorbarh(height=.1) + scale_x_continuous(limits=c(0,5), breaks = c(0:5), name=xname) +
#   geom_vline(xintercept=0, color="black", linetype="dashed", alpha=.5) + 
#   facet_grid(set~., scales= "free", space="free") + ggtitle("PPI cliques")+
#   theme_minimal() + theme(text=element_text(family="Times",size=18, color="black"))+
...
# p + coord_flip()
# 
# ##using library(metafor)
# library(metafor)
# help(metafor)
# #forest(x = df1_disc$OR_Fish, ci.lb = df1_disc$CI_lower, ci.ub = df1_disc$CI_upper)
# forest(CI_comb_meta_df$Fish_OR_meta, CI_comb_meta_df$lower.CI, CI_comb_meta_df$upper.CI)
...
p_meta <- ggplot(data=CI_comb_MH_meta_df, aes(y=cmpx_name, x=log(Fish_OR_meta), xmin=log(lower.CI), xmax=log(upper.CI))) +
  ...
  geom_errorbarh(height=.1) + scale_x_continuous(limits=c(-1,5), breaks = c(-1:5), name=xname) +
  geom_vline(xintercept=0, color="black", linetype="dashed", alpha=.5) +
  facet_grid(set~., scales= "free", space="free") + ggtitle("PPI cliques")+
  theme_minimal() + theme(text=element_text(family="Times",size=18, color="black"))+
  theme(panel.spacing = unit(1, "lines"))
