...
write.table(ppi_res_fil_final, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/SKAT/ppi_res_fil_final_SKATbin_Aug31_sep19.tsv", sep = "\t", row.names = F, quote = F)
...
ppi_res_fil_final <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/SKAT/ppi_res_fil_final_SKATbin_Aug31_sep19.tsv", sep = "\t", header = T)
...
# can_net_graph1 <- strindb_biog_graph1
can_net1 <- igraph::as_data_frame(can_net_graph1, what = "edges")
...
strindb_graph_cyto <- readRDS("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/SKAT/strindb_cyto_graph.rds")
 ...
 # strindb_graph_cyto_df <- igraph::as_data_frame(strindb_graph_cyto, what = "edges")

# ##combine biogrid and stringdb
# colnames(strindb_graph_cyto_df) <- c("node1", "node2")
# string_biog <- unique(rbind.data.frame(strindb_graph_cyto_df,can_net))
# strindb_biog_graph <- igraph::graph.data.frame(string_biog, directed = F)
# strindb_biog_graph_cyto <- igraph::simplify(strindb_biog_graph, remove.loops=T, remove.multiple = T)

# saveRDS(strindb_biog_graph_cyto, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/SKAT/strindb_biog_graph_cyto.rds", compress = T)
...
# strindb_biog_graph1_rect1 <- igraph::simplify(strindb_biog_graph1_rect, remove.loops=T, remove.multiple = T)
...
#write.table(ppi_res_fil_final_comb, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/SKAT/ppi_res_fil_final_SKATbin_comb_str_biog_Aug31_cyto.tsv", sep = "\t", row.names = F, quote = F)
write.table(ppi_res_fil_final_comb, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/SKAT/ppi_res_fil_final_SKATbin_comb_str_biog_Aug31_sep19_cyto.tsv", sep = "\t", row.names = F, quote = F)
...
ppi_res_fil_final_comb[is.na(ppi_res_fil_final_comb$eigen_wt_biog) | ppi_res_fil_final_comb$eigen_wt_biog == 0,]$eigen_wt_biog <- sample(seq(0.00001, 0.0001, 0.001), size = dim(ppi_res_fil_final_comb[is.na(ppi_res_fil_final_comb$eigen_wt_biog) | ppi_res_fil_final_comb$eigen_wt_biog == 0,])[1], replace = T)

set.seed(233345561)
#ppi_res_fil_final_comb[ppi_res_fil_final_comb$degree_str == 0,]$eigen_wt_str <- sample(seq(0.00001, 0.0001, 0.001), size = 672, replace = T)
ppi_res_fil_final_comb[is.na(ppi_res_fil_final_comb$eigen_wt_str) | ppi_res_fil_final_comb$eigen_wt_str == 0,]$eigen_wt_str <- sample(seq(0.00001, 0.0001, 0.001), size = dim(ppi_res_fil_final_comb[is.na(ppi_res_fil_final_comb$eigen_wt_str) | ppi_res_fil_final_comb$eigen_wt_str == 0,])[1], replace = T)

set.seed(233345563)
#ppi_res_fil_final_comb[ppi_res_fil_final_comb$degree_str_biog == 0,]$eigen_wt_str_biog <- sample(seq(0.00001, 0.0001, 0.001), size = 419, replace = T)
ppi_res_fil_final_comb[is.na(ppi_res_fil_final_comb$eigen_wt_str_biog) | ppi_res_fil_final_comb$eigen_wt_str_biog == 0,]$eigen_wt_str_biog <- sample(seq(0.00001, 0.0001, 0.001), size = dim(ppi_res_fil_final_comb[is.na(ppi_res_fil_final_comb$eigen_wt_str_biog) | ppi_res_fil_final_comb$eigen_wt_str_biog == 0,])[1], replace = T)

# set.seed(23334556)
# ppi_res_fil_final_comb[ppi_res_fil_final_comb$eigen_wt_biog == 0,]$eigen_wt_biog <- sample(seq(0.00001, 0.0001, 0.001), size = 6, replace = T)
# set.seed(233345561)
# ppi_res_fil_final_comb[ppi_res_fil_final_comb$eigen_wt_str == 0,]$eigen_wt_str <- sample(seq(0.00001, 0.0001, 0.001), size = 103, replace = T)
# set.seed(233345563)
# ppi_res_fil_final_comb[ppi_res_fil_final_comb$eigen_wt_str_biog == 0,]$eigen_wt_str_biog <- sample(seq(0.00001, 0.0001, 0.001), size = 12, replace = T)

set.seed(233345563)
ppi_res_fil_final_comb[ppi_res_fil_final_comb$degree == 0,]$PPI_p_val_wt <- sample(seq(0.990, 0.999, 0.00001), size = 572, replace = F)
set.seed(233345563)
ppi_res_fil_final_comb[ppi_res_fil_final_comb$degree_str == 0,]$PPI_p_val_wt_str <- sample(seq(0.990, 0.999, 0.00001), size = 672, replace = F)
set.seed(233345563)
ppi_res_fil_final_comb[ppi_res_fil_final_comb$degree_str_biog == 0,]$PPI_p_val_wt_str_biog <- sample(seq(0.990, 0.999, 0.00001), size = 419, replace = F)
...
##ignore this and refer cliq_analysis sub-directory
...
t1_cliq4_IDs <- t1[t1$c1_perc_ov >= 0.75,1:2]
...
##Add eigen_centrality scores; necessary for p-value calculation
library(reshape2)
eigen_cliq <- lapply(te3, function(x)sum(ppi_res_fil_final_comb[ppi_res_fil_final_comb$gene %in% names(x),40]))
eigen_cliq_df <- melt(eigen_cliq)
cliq_size_lab <- melt(sapply(te3, length))
cliq_size_lab$cliq <- rownames(cliq_size_lab)
eigen_cliq_df$cliq_size <- cliq_size_lab[match(eigen_cliq_df$L1, cliq_size_lab$cliq), 1]
eigen_cliq_df <- eigen_cliq_df[order(eigen_cliq_df$value, decreasing = T),]
##sample at random 1176(or whatever is the number) 1000X from the entire list of genes in strindb_biog_graph1 and calculate null distribution of eigen scores for all the clique sizes of interest or sample at random from the 1176 list itself using the cliq size as size of sample; compute p-value using the null distribution and the score statistic in eigen_cliq

eigen_null <- function(clique_size){
clique_eigen <- t(replicate(n=50, expr = ppi_res_fil_final_comb[ppi_res_fil_final_comb$gene %in% top_genes[sample(1:length(top_genes), size = clique_size, replace = F)], 40]))
return(clique_eigen)
}
eigen_cliq_pval <- list()
for(k in 1:length(cliq_size)){
eigen_null_dist <- eigen_null(cliq_size[k])
eigen_cliq_df_sub <- eigen_cliq_df[eigen_cliq_df$cliq_size == cliq_size[k], ]
empcdf <- ecdf(rowSums(eigen_null_dist))
pval_emp <- 1 - empcdf(eigen_cliq_df_sub$value)
eigen_cliq_df_sub$pval_emp <- pval_emp
eigen_cliq_pval[[k]] <- eigen_cliq_df_sub
#hist(rowSums(eigen_null_dist))
}

eigen_cliq_pval_df <- do.call("rbind.data.frame", eigen_cliq_pval)
cliq_genes <- lapply(te3, function(x)paste(names(x), collapse = ","))
cliq_genes_df <- melt(cliq_genes)
eigen_cliq_pval_df$genes <- cliq_genes_df[match(eigen_cliq_pval_df$L1, cliq_genes_df$L1), 1]
...
cpx <- unique(unlist(strsplit(as.character(eigen_cliq_pval_df[eigen_cliq_pval_df$pval_emp > 0.1,]$genes), ",")))
cpx_np_all <- unique(can_net1[as.character(can_net1$from) %in% cpx 
                               & as.character(can_net1$to) %in% cpx, ])
cpx_graph <- igraph::graph.data.frame(cpx_np_all, directed = F)
cpx_graph <- igraph::simplify(cpx_graph, remove.loops=T, remove.multiple = T)
...
##clique p-value based on burden (Fisher test) and weighted burden(SKAT) test
```{r}
...
##parallelised test for non-overlapping cliques (cliques overlap < 0.3)
