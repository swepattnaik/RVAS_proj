...
#genomicclass
comb_pheno_no_comp <- as.character(comb_ALL_phen[comb_ALL_phen$genomicclass %nin% "Simple",]$pmn)
##18 cases not annotated in Mandy's data
not_annot <- Ex_samp_id[Ex_samp_id %nin% comb_ALL_phen[comb_ALL_phen$genomicclass %in% "Simple",]$pmn & !grepl("^[ABZ]", Ex_samp_id)]
...
get_coh_dist_para <- function(gene_sym, fil_db){
  ...
  if(length(ftemp_tab_var_id) == 0 ){ ##added on apr.22
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      ...
      #maf_vec_cont <- sum(ifelse(is.na(as.numeric(sam_gene_gt$SAMPLE)), 1, 0))/((371 + 3205)*2)
      ##it should be for MGRB and to not penalise CR's and LK's:Sep18-2020
      maf_vec_cont <- length(grep("^[ABZ]", sam_gene_gt$SAMPLE))/((371 + 3205)*2)
      maf_vec_case <- sum(ifelse(!is.na(as.numeric(sam_gene_gt$SAMPLE)) | 
                                   grepl("^CR|^LK", as.character(sam_gene_gt$SAMPLE)), 1, 0))/((371 + 3205)*2)
...
saveRDS(res20, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Simple/Exome_para_ISKS_SIMPLE_2020_minusC3_Aug31.rds", compress = T)
