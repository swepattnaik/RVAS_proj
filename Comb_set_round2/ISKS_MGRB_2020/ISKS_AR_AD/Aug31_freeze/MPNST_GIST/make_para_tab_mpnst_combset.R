...
#sarcomatype
comb_pheno_no_comp <- as.character(comb_ALL_phen[comb_ALL_phen$sarcomatype %nin% c("MPNST","GIST"),]$pmn)
##18 cases not annotated in Mandy's data
not_annot <- Ex_samp_id[Ex_samp_id %nin% comb_ALL_phen[comb_ALL_phen$sarcomatype %in% c("MPNST","GIST"),]$pmn & !grepl("^[ABZ]", Ex_samp_id)]
...
##complex class
# comb_pheno_cpx <- as.character(comb_ALL_phen[comb_ALL_phen$genomicclass %in% "Complex",]$pmn)
# 
# fil_tab_mgrb <- fil_tab_noCH[grep("^MGRB", fil_tab_noCH$set), ]
# fil_tab_isks <- fil_tab_noCH[grep("^ISKS", fil_tab_noCH$set), ]
# fil_tab_isks_cpx <- fil_tab_isks[fil_tab_isks$SAMPLE %in% comb_pheno_cpx,]
# fil_tab_isks_cpx_mgrb <- rbind.data.frame(fil_tab_isks_cpx, fil_tab_mgrb)
...
get_coh_dist_para <- function(gene_sym, fil_db){
  ...
  if(length(ftemp_tab_var_id) == 0 ){ ##added on apr.22
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      ...
      #maf_vec_cont <- sum(ifelse(is.na(as.numeric(sam_gene_gt$SAMPLE)), 1, 0))/((157 + 3205)*2)
      ##it should be for MGRB and to not penalise CR's and LK's:Sep18-2020
      maf_vec_cont <- length(grep("^[ABZ]", sam_gene_gt$SAMPLE))/((157 + 3205)*2)
      maf_vec_case <- sum(ifelse(!is.na(as.numeric(sam_gene_gt$SAMPLE)) | 
                                   grepl("^CR|^LK", as.character(sam_gene_gt$SAMPLE)), 1, 0))/((157 + 3205)*2)
      ##MAF filter = 5/(1661*2); change to 3/(1661*2)
      ##changed to cohort MAF threshold : 3/((157 + 3205)*2) 
      if(!(as.numeric(as.character(maf_vec_cont)) >= 0.00045 | 
           as.numeric(as.character(maf_vec_case)) >= 0.00045)){
        var_vec[[m]] <- ftemp_tab_var_id[m]
      }
      else{ 
        var_vec_rm[[m]] <- ftemp_tab_var_id[m]
        next 
      }
    }
  }
  ...
  res <- cbind.data.frame("ISKS_mpnst" = isks, "MGRB" = control, "gene" = gene_sym, 
...
saveRDS(res20, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/MPNST/Exome_para_ISKS_MPNST_sep30.rds", compress = T)
