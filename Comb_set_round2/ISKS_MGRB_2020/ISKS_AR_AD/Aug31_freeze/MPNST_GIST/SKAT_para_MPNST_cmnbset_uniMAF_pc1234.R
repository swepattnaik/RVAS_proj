...
#  print(genes[i])
  ...
  if(dim(ftemp_tab) == 0 ){
    ...
    ## change to entire cohort MAF filter to 3/((157+3205)*2)##latest Aug.31
    ...
    samp_vec_mat <- samp_vec_mat[!(as.numeric(as.character(samp_vec_mat$coh_MAF_case)) >= 0.00045 | 
                                     as.numeric(as.character(samp_vec_mat$coh_MAF_cont)) >= 0.00045),]
...
saveRDS(res_skat_para, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/SKAT_intra/Exome_skat_MPNST_combset2020_uni_MAF_PC1234_ver4_clinrect_Aug31.rds", compress = T)
