...
#Ex_tab_mgrb_hart_genes <- Ex_tab_mgrb[Ex_tab_mgrb$gene_symbol %in% genes4mgrb & Ex_tab_mgrb$auto_call %nin% "C3",]
...
#aug_all_hart_genes[aug_all_hart_genes$aug_var > 0,]
#############
...
cpx_repset_839_POIS_noC3 <- do.call("rbind.data.frame", cpx_OR_fisher(aug_all_hart_genes, 839, 3205, "rep839vsMGRB"))
#cpx_repset_839_POIS_noC3_sp <- do.call("rbind.data.frame", cpx_OR_fisher(aug_all_hart_genes_sp, 839, 3205, "rep839vsMGRB"))
...
noC3_df_comb_hart <- noC3_df_comb_hart[noC3_df_comb_hart$gene %in% c("BRCA_genes","Shelterin", "CEP_HAUS", "SARC_genes"),]
