...
# 
# ##For supp figure
# CI_comb_meta_df_sub <- CI_comb_meta_df[CI_comb_meta_df$cmpx_name %in% c("SARC_genes", "BRCA_genes",
#                                                                         "TP53_control", "Shelterin",
#                                                                         "CEP_HAUS_core", "MPNST_pos"),]
# 
# p_sub <- ggplot(data=CI_comb_meta_df_sub, aes(y=cmpx_name, x=log2(Fish_OR_meta), xmin=log2(lower.CI), xmax=log2(upper.CI))) + 
#   geom_point() + geom_point(data=subset(CI_comb_meta_df_sub, set=="Disc"), color="Black", size=2) + 
#   geom_errorbarh(height=.1) + scale_x_continuous(limits=c(0,5), breaks = c(0:5), name=xname) +
#   geom_vline(xintercept=0, color="black", linetype="dashed", alpha=.5) + 
#   facet_grid(set~., scales= "free", space="free") + ggtitle("PPI cliques")+
#   theme_minimal() + theme(text=element_text(family="Times",size=18, color="black"))+
...
# p_sub
# 
# p_sub1 <- ggplot(data=CI_comb_meta_df_sub, aes(y=cmpx_name, x=Fish_OR_meta, xmin=lower.CI, xmax=upper.CI)) + 
#   geom_point() + geom_point(data=subset(CI_comb_meta_df_sub, set=="Disc"), color="Black", size=2) + 
#   geom_errorbarh(height=.1) + scale_x_continuous(limits=c(0,17), breaks = c(0:17), name=xname) +
#   geom_vline(xintercept=0, color="black", linetype="dashed", alpha=.5) + 
#   facet_grid(set~., scales= "free", space="free") + ggtitle("PPI cliques")+
#   theme_minimal() + theme(text=element_text(family="Times",size=18, color="black"))+
#   theme(panel.spacing = unit(1, "lines"))
