...
rep_set <- do.call("rbind.data.frame", lapply(t1000, function(x)x[[2]]))
  ...
  write.table(disc_set, paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Validation/",
                               names(cpx_list)[k],"_fisher.disc.tsv"), sep = "\t", quote = F, row.names = F)
  write.table(rep_set, paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Validation/",
...
plot_val_data <- function(cmpx_name,plot_type = c("box", "dens")){
  dens_plot <- list()
  box_plots <- list()
df1_disc <- read.delim(paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Validation/",cmpx_name,"_fisher.disc.tsv"),
                       sep = "\t", header = T, stringsAsFactors = F )
df1_rep <- read.delim(paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Aug31/Validation/",cmpx_name,"_fisher.rep.tsv"),
                      sep = "\t", header = T, stringsAsFactors = F )
