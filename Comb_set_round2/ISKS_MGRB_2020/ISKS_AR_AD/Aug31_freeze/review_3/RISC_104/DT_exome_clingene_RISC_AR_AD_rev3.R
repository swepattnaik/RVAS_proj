...
Ex_tab <- read.delim("~/RVAS/RISC_104/all_shards_RISC104_gnomadAF05.tsv", sep = "\t",
                     header = T, stringsAsFactors = F)
...
table(t1 < 0) ##all values must be positive since mutated position is less than the length of the protein
...
Ex_tab_noCH_filt3 <- Ex_tab_filt2
rm(Ex_tab_filt2)
...
Ex_tab_noCH_filt3_fin_filt$set <- c("GIST_AR_AD")


write.table(Ex_tab_noCH_filt3_fin_filt, file = "~/RVAS/RISC_104/RISC104_variants_AR_AD_all_fields_clingene_rev3_freeze.tsv",
            row.names = F, quote = F, sep = "\t")

Ex_tab_noCH_filt3_fin_filt = read.delim("~/RVAS/RISC_104/RISC104_variants_AR_AD_all_fields_clingene_rev3_freeze.tsv", sep = "\t", header = T, stringsAsFactors = F)
Ex_tab_noCH_filt3_fin_filt = Ex_tab_noCH_filt3_fin_filt[!is.na(Ex_tab_noCH_filt3_fin_filt$SAMPLE),]
##complex genes
...
Ex_tab_noCH_filt3_fin_filt_sub = Ex_tab_noCH_filt3_fin_filt[Ex_tab_noCH_filt3_fin_filt$gene_symbol %in% genes,]
##gnomad_AF_NFE filter

Ex_tab_noCH_filt3_fin_filt_sub = Ex_tab_noCH_filt3_fin_filt_sub[as.numeric(Ex_tab_noCH_filt3_fin_filt_sub$gnomad_AF_NFE) <= 0.0002,]

##Centrosome genes

Ex_tab_noCH_filt3_fin_filt_centro = Ex_tab_noCH_filt3_fin_filt_sub[Ex_tab_noCH_filt3_fin_filt_sub$gene_symbol %in% CEP_HAUS_core,]

##CHIP genes

chip_genes = c("TERT", "DNMT3A", "TET2", "NPM1", "IDH2", "RUNX1")

Ex_tab_noCH_filt3_fin_filt_chip = Ex_tab_noCH_filt3_fin_filt[Ex_tab_noCH_filt3_fin_filt$gene_symbol %in% chip_genes,]

##no mutations in CHIP genes
dim(Ex_tab_filt1[Ex_tab_filt1$gene_symbol %in% chip_genes,])

table(Ex_tab_filt1[Ex_tab_filt1$gene_symbol %in% chip_genes,]$vep_consequence)

##CEP-HAUS Extended complex based on C4/5 mutations in discovery set
Ex_tab_noCH_filt3_fin_filt[Ex_tab_noCH_filt3_fin_filt$gene_symbol %in% CEP_HAUS_core_extn$x,]
...
fil_tab[fil_tab$gene_symbol %in% CEP_HAUS_core_extn$x & fil_tab$auto_call %nin% "C3" ,c(1,9,127)]
table(fil_tab[fil_tab$gene_symbol %in% CEP_HAUS_core_extn$x & fil_tab$auto_call %nin% "C3" ,]$set)

table(fil_tab[fil_tab$gene_symbol %in% CEP_HAUS_core_extn2 & fil_tab$auto_call %nin% "C3" ,]$set)

##fisher's test: note ISKS (n = 1631); MGRB (n = 3205)
inp = c(43, 1588, 82, 3123)
inp = c(28, 1603, 48, 3157)
