...
rep_set <- do.call("rbind.data.frame", lapply(t1000, function(x)x[[2]]))
  ...
  write.table(disc_set, paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Validation/",
                               names(cpx_list)[k],"_fisher.disc.tsv"), sep = "\t", quote = F, row.names = F)
  write.table(rep_set, paste0("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/ISKS_AR_AD/Validation/",
