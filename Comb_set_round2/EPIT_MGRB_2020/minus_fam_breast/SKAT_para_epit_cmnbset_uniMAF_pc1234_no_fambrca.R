...
#epit_pheno <- epit_pheno[epit_pheno$Study %nin% "Familial breast",]
...
##save table
#write.table(fil_tab, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/all_epit_mgrb_combset2020_variants_filt_all_fields_nof_brca.tsv", 
...
#  print(genes[i])
  ...
  if(dim(ftemp_tab) == 0 ){
    ...
    samp_vec_mat <- samp_vec_mat[!(as.numeric(as.character(samp_vec_mat$coh_MAF_case)) >= 0.000371 | 
                                     as.numeric(as.character(samp_vec_mat$coh_MAF_cont)) >= 0.000371),]
...
saveRDS(res_skat_para, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/round2/Exome_skat_para_result_EPIT_combset2020_uni_MAF_PC1234_ver4_no_fbrca.rds", compress = T)
