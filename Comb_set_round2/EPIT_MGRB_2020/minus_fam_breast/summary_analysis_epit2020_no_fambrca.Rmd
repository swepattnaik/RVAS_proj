...
fil_tab_noCH <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/all_epit_mgrb_combset2020_variants_filt_all_fields_nof_brca.tsv", sep = "\t", header = T, stringsAsFactors = F)
##remove 78 familial breast cancer cases
#`%nin%` = Negate(`%in%`)
#epit_pheno <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/git_proj_rvas/Comb_set/EPIT_MGRB_2020/qc0_final_all_annot.tsv",sep = "\t", header = T, stringsAsFactors = F)
#epit_pheno_fam_brca <- epit_pheno[epit_pheno$Study %in% "Familial breast",]$Sample_Id
#fil_tab_noCH <- fil_tab_noCH[fil_tab_noCH$SAMPLE %nin% epit_pheno_fam_brca,]
...
write.table(pc123_gender_tab_top1000_VAR, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/round2/Exome_Enriched_pc_gender_controlled_top_geneVARIANTS_EPIT_pval01_no_fbrca.tsv", sep = "\t", row.names = F, quote = F)
...
##Use PPI approach to derive differential enrichment of mutated interactomes in Epithelial dataset.
```{r, out.width='\\textwidth', fig.height = 7, fig.align='center', echo=FALSE}
...
write.table(ppi_res_fil_final, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/round2/ppi_res_fil_final_EPIT_no_fbrca.tsv", sep = "\t", row.names = F, quote = F)
```
...
para_fisher_weighted_new <- function(gene_sym){
  ...
  ##simple Fisher test
  #inp <- c(deg_union, deg_biog, tot_union - deg_union, tot_biog - deg_biog)
  ##without 78 familial breast cases
  inp <- c(ppi_res[,1], 842 - ppi_res[,1] , ppi_res[,2], 3205 - ppi_res[,2])
  ...
  ##Weighted Fisher
  inp1 <- c(ppi_res[,4], 842 - ppi_res[,1] , ppi_res[,5], 3205 - ppi_res[,2])
...
write.table(ppi_res_fil_final, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/round2/ppi_res_fil_final_EPIT_SKATbin_wt_fisher_no_fbrca.tsv", sep = "\t", row.names = F, quote = F)
...
saveRDS(goNet_enrich_res, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/round2/goNet_enrich_res_sep05_combset2020_EPIT_no_fbrca.rds", compress = T)
...
write.table(fil_graph_final_comb_GO, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/round2/EPIT_combset2020_final_PPI_comb_GO_no_fbrca.tsv", sep = "\t", row.names = F, quote = F)
