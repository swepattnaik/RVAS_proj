...
get_coh_dist_para <- function(gene_sym, fil_db){
  ...
  if(length(ftemp_tab_var_id) == 0 ){ ##added on apr.22
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      ...
      maf_vec_cont <- dim(sam_gene_gt[grepl("^[ABZ]",sam_gene_gt$SAMPLE),])[1]/((842 + 3205)*2)
      maf_vec_case <- dim(sam_gene_gt[!grepl("^[ABZ]",sam_gene_gt$SAMPLE),])[1]/((842 + 3205)*2)
      ##MAF filter = 5/(1661*2); change to 3/(1662*2)
      ##changed to cohort MAF threshold : 3/((920 + 3209)*2) 
      if(!(as.numeric(as.character(maf_vec_cont)) >= 0.000371 | 
           as.numeric(as.character(maf_vec_case)) >= 0.000371)){
  ...
  res_df <- cbind.data.frame("EPIT" = isks, "MGRB" = control, "gene" = gene_sym, 
...
saveRDS(res20, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/round2/Exome_para_pc123_SKAT_Enriched_EPIT_2020_no_fbrca_minusC3.rds", compress = T)
