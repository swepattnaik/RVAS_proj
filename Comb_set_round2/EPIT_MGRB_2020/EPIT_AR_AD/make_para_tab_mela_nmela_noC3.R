...
`%nin%` = Negate(`%in%`)
fil_tab_mela <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT_AR_AD/Tabs/EPIT_melanoma_skat_inp_tab.tsv",
                           sep = "\t", header = T, stringsAsFactors = F)
Ex_samp_mela <- unique(fil_tab_mela$SAMPLE)
fil_tab_nmela <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT_AR_AD/Tabs/EPIT_non_melanoma_skat_inp_tab.tsv",
                           sep = "\t", header = T, stringsAsFactors = F)
Ex_samp_nmela <- unique(fil_tab_nmela$SAMPLE)

get_coh_dist_para <- function(gene_sym, fil_db, samp_ids){
  coh_size <- length(samp_ids)
  maf_cutoff <- 3/(2*coh_size)
  #res <- list()
  ...
  if(length(ftemp_tab_var_id) == 0 ){ ##added on apr.22
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      ...
      maf_vec_cont <- dim(sam_gene_gt[grepl("^[ABZ]",sam_gene_gt$SAMPLE),])[1]/(coh_size*2)
      maf_vec_case <- dim(sam_gene_gt[!grepl("^[ABZ]",sam_gene_gt$SAMPLE),])[1]/(coh_size*2)
...
genes <- unique(fil_tab_mela$gene_symbol)
system.time(res_list <- foreach(i=1:length(genes), .errorhandling = 'remove') %dopar% 
{get_coh_dist_para(genes[i], fil_tab_mela, Ex_samp_mela)})
res20 <- do.call("rbind.data.frame", res_list)


saveRDS(res20, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT_AR_AD/SKAT/Exome_para_EPIT_mela_2020_noC3.rds", compress = T)


res_list <- list()
#  genes <-  as.character(Exome_pc123_srt_SKAT[[k]][,1])
genes <- unique(fil_tab_nmela$gene_symbol)
system.time(res_list <- foreach(i=1:length(genes), .errorhandling = 'remove') %dopar% 
{get_coh_dist_para(genes[i], fil_tab_nmela, Ex_samp_nmela)})
res_nmela <- do.call("rbind.data.frame", res_list)


saveRDS(res_nmela, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT_AR_AD/SKAT/Exome_para_EPIT_nmela_2020_noC3.rds", compress = T)
