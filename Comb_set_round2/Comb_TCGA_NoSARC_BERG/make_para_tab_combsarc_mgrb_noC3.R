...
`%nin%` = Negate(`%in%`)
fil_tab_noCH <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/Comb_TCGA_BERG_NoSarc/all_tcga_nosarc_berg_mgrb_skatinp_combset2020_clin_C3C4C5_NFE0002_AD_all_fields_rnd3_Aug31.tsv",
...
get_coh_dist_para <- function(gene_sym, fil_db){
  ...
  fil_db_genes <- fil_db_genes[fil_db_genes$comb_score >= 5.6 & fil_db_genes$VAF >= 0.35 & fil_db_genes$auto_call %nin% "C3",]
  ...
  if(length(ftemp_tab_var_id) == 0 ){ ##added on apr.22
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      ...
      if(!(as.numeric(as.character(maf_vec_cont)) >= 0.0005 | 
           as.numeric(as.character(maf_vec_case)) >= 0.0005)){
...
saveRDS(res20, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/Comb_TCGA_BERG_NoSarc/Exome_para_CombSarc_minusC3Aug31_sep19.rds", compress = T)
#saveRDS(res20, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/Comb_TCGA_BERG_NoSarc/Exome_para_CombSarc_minusC3Aug31.rds", compress = T)
