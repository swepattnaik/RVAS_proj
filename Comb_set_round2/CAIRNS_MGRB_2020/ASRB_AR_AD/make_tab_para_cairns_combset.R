 ...
 #   print(i)
    ...
    if(length(ftemp_tab_var_id) == 0 ){ ##added on apr.22
      ...
      for(m in 1:length(ftemp_tab_var_id)){
        ...
        maf_vec_cont <- dim(sam_gene_gt[grepl("^[ABZ]",sam_gene_gt$SAMPLE),])[1]/((406 + 3205)*2)
        maf_vec_case <- dim(sam_gene_gt[!grepl("^[ABZ]",sam_gene_gt$SAMPLE),])[1]/((406 + 3205)*2)
...
saveRDS(res20, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/CAIRNS/round2/ASRB_AR_AD/SKAT/Exome_para_pc123_SKAT_Enriched_CAIRNS_2020.rds", compress = T)
