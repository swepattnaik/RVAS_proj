...
mgrb_tsv <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/MGRB/all_shards_MGRB_combset2020.tsv", sep = "\t",
                       header = T, stringsAsFactors = F)
Ex_tab <- rbind.data.frame(cairns_tsv, mgrb_tsv)
rm(cairns_tsv, mgrb_tsv)
...
Ex_tab_filt2 <- Ex_tab1[!(grepl(paste(toMatch,collapse="|"), 
                                     Ex_tab1$vep_consequence)),]
...
#Ex_tab_noCH_filt3_fin <- Ex_tab_noCH_filt3_fin[,c(1:9,11,15,17:18,20,23:25,30,53,57,59:68,72,124:132)]
...
#  sam_var_vec <- ifelse(!is.na(as.numeric(as.character(sam_var$SAMPLE))) | grepl("^CR|^LK", as.character(sam_var$SAMPLE)), 1, 0)
  sam_var_vec <- ifelse(grepl("^[ABZ]",as.character(sam_var$SAMPLE)), 0, 1)
  print(length(sam_var_vec))
  MAF_case <- sum(sam_var_vec)/(2*453)
...
Ex_tab_noCH_filt3_fin_filt$is_case <- ifelse(grepl("^[ABZ]",as.character(Ex_tab_noCH_filt3_fin_filt$SAMPLE)), 0, 1)
Ex_tab_noCH_filt3_fin_filt$MAF_case <- ifelse((is.na(Ex_tab_noCH_filt3_fin_filt$MAF_case) 
                                               & Ex_tab_noCH_filt3_fin_filt$is_case == 1),
                                              1/(2*453), Ex_tab_noCH_filt3_fin_filt$MAF_case)
...
#write.table(Ex_tab_noCH_filt3_fin_filt[,c(1:9,11,15,17:18,20,23:25,30,53,57,59:68,72,119,124:132)], file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/EPIT/all_epit_mgrb_combset2020_variants_filt_all.tsv",
#            row.names = F, quote = F, sep = "\t")
write.table(Ex_tab_noCH_filt3_fin_filt, file = "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/CAIRNS/all_cairns_mgrb_combset2020_variants_filt_all_fields_risk_factor.tsv",
            row.names = F, quote = F, sep = "\t")
