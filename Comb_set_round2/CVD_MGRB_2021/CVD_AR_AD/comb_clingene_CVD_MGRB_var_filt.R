...
Ex_tab_cvd <- read.delim("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/CVD_DIANE/round2/CVD_AR_AD/all_CVD_combset2020_variants_AR_AD_all_fields_clingene_rnd3.tsv", sep = "\t", header = T, stringsAsFactors = F)
Ex_tab_mgrb$set <- gsub("ISKS", "MGRB", Ex_tab_mgrb$set)

Ex_tab <- rbind.data.frame(Ex_tab_mgrb, Ex_tab_cvd)
...
##This is 0.012 for CVD cohort (3/(117*2)); use 0.015 (warning: this will permit upto 50 variants in MGRB)
##but its dealt with in the SKAT algorithm
Ex_tab_C4C5 <- Ex_tab_C4C5[Ex_tab_C4C5$cohort_MAF <= 0.015 & Ex_tab_C4C5$swegen_AF <= 0.001,]
##C3; gnomad_AF == 0
...
##cohort_MAF <= 0.001
##This is 0.012 for CVD cohort (3/(117*2))
Ex_tab_C3 <- Ex_tab_C3[Ex_tab_C3$cohort_MAF <= 0.015 & Ex_tab_C3$comb_score >= 5.6,]
...
write.table(Ex_tab_C3C4C5_AD_filt, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/comb_set_2020/CVD_DIANE/round2/CVD_AR_AD/all_CVD_mgrb_skatinp_combset2021_clin_C3C4C5_NFE0002_AD_all_fields_rnd3.tsv", 
            row.names = F, quote = F, sep = "\t")
