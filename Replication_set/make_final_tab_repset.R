...
fil_tab_noCH <- readRDS("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/Rep_set_variants/Rep_set_2044_C5eqC4_nonmds_iskrisclions_Oct08_rect_ASP_new_score.rds")
...
get_coh_dist <- function(gene_sym, fil_db){
  ...
  for(i in 1:length(gene_sym)){
    ...
    for(m in 1:length(ftemp_tab_var_id)){
      ...
      if(!(as.numeric(as.character(maf_vec_cont)) >= 0.0011 | 
           as.numeric(as.character(maf_vec_case)) >= 0.0026)){
...
Exome_skat <- readRDS("~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/Rep_set_variants/Exome_skat_wsing_load123_noCH_C5eqC4_repset_pc4_nov2019.rds")
...
saveRDS(Exome_pc123_srt_SKAT_case_enr_nCH, "~/RVAS/shard_sub_tier3/DT_sheet/EXOME_isks_risc/test/Rep_set_variants/Exome_pc123_srt_SKAT_case_enr_nCH_repset_08Oct_rect_ASP_cmaf_new_score_pc4.rds", compress = T)
